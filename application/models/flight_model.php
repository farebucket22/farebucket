...
class flight_model extends CI_Model{
    ...
    function fetchAllTickets(){
        $data = $this->db->get('farebucket_ticket_save');
        if( $data->num_rows() > 0 ){
            return $data->result();
        }else{
            return false;
    ...
    function updateDepAndArrTime($id, $date){        
        $this->db->where('id', $id);
        if($this->db->update('farebucket_ticket_save', array('DepTime' => $date))){
            return true;
        }
        else{
            return false;
    ...
    function fetchTicketDetailsByBookingId($bookingid)
    {
      $data = $this->db->get_where("farebucket_ticket_save", array('fbBooking_id' => $bookingid));      
      if($data){
            return $data->result(); 
        }
        else{
    ...
    public function getLastFbBookingId(){
    	$data = $this->db->query('SELECT fb_bookingId FROM farebucket_ticket_confirmation ORDER BY fb_bookingId DESC LIMIT 0,1');
    	$res = $data->result();
    	if(!isset($res[0])){
            $fbId = 0;
            return $fbId;
