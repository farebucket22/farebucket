...
class cab_model extends CI_Model{
	...
	function insert_details($total_cars,$guest_info,$ref_id,$type,$sum,$total_fare,$cab_min_slab, $offered_price, $published_price, $convenience_charge)
	{
		$user_id = 0;
		...
		$guest_info['offered_price'] = $offered_price;
		$guest_info['published_price'] = $published_price;
		$guest_info['convenience_charge'] = $convenience_charge;
	...
	function insert_booking_details($total_cars,$guest_info,$ref_id,$type,$sum,$total_fare,$cab_min_slab, $offered_price, $published_price, $convenience_charge)
	{
		$user_id = 0;
		if( isset($_SESSION['user_details'][0]->user_id) ){
			$user_id = $_SESSION['user_details'][0]->user_id;
    ...
    function getLastFbBookingId(){
    	$data = $this->db->query('SELECT fb_bookingId FROM farebucket_cab_details ORDER BY fb_bookingId DESC LIMIT 0,1');
    	$res = $data->result();
    	if(!isset($res[0])){
    		$fbId = 0;
    		return $fbId;
