...
class Activity_Model extends CI_Model{
    ...
    public function update_booking_status($bookingId, $status){
        ...
        $this->db->where('fb_bookingId', $bookingId);
    ...
    public function get_ticket_by_booking_id($booking_id) {
        $data = $this->db->get_where("farebucket_activity_booking", array('fb_bookingId'=>$booking_id));
        return $data->result();
    }

    public function get_activity_phone_num_by_id($activity_id) {
        $data = $this->db->get_where("farebucket_activity", array('activity_id'=>$activity_id));
        return $data->result()[0]->activity_phone_num;
    }

    public function get_activity_address_by_id($activity_id) {
        $data = $this->db->get_where("farebucket_activity", array('activity_id'=>$activity_id));
        return $data->result()[0]->activity_address;
    }
    ...
    public function get_activity_details_by_id($activity_id) {
        $data = $this->db->get_where("farebucket_activity", array('activity_id'=>$activity_id));
        return $data->result()[0]->activity_details;
    }
    ...
    public function update_actvity_status($booking_id){
        ...
        if($difference > 11)
        {
            if($this->db->update('farebucket_activity_booking',array('booking_status' => "Cancelled"),array('booking_id' => $booking_id))){
                $this->activity_update_time($booking_id);
                return true;
            }
        }
        else
    ...
    public function getLastFbBookingId(){
        $data = $this->db->query('SELECT fb_bookingId FROM farebucket_activity_booking ORDER BY fb_bookingId DESC LIMIT 0,1');
        $res = $data->result();
        if(!isset($res[0])){
            $fbId = 0;
            return $fbId;
    ...
    public function getActivityByUserDetails($query){
        $data = $this->db->get_where("farebucket_activity_booking", $query);
        if( $data->num_rows() > 0 ){
            return $data->result();
        }
    ...
    public function activity_update_time($booking_id){
        $this->db->query("UPDATE `farebucket_activity_booking` SET `booking_update_date_time` = NOW() WHERE `booking_id` = ".$booking_id);
    }
}
?>