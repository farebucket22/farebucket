...
class Flights extends MY_Controller
{
    function search_flights_return()
    {
		...
		if((isset($_SESSION['oneway_utf_from']))) {
            $data['data']['utf_from'] = $_SESSION['oneway_utf_from'];
        }
        if((isset($_SESSION['oneway_utf_to'])) ) {
            $data['data']['utf_to'] = $_SESSION['oneway_utf_to'];
        }

        $wsdl = "http://airapi.travelboutiqueonline.com/tboapi_v7/service.asmx?wsdl";
        $headerpara = array();
        $headerpara["UserName"] = 'PNYR196';
        $headerpara["Password"] = 'travel/090';
        $client_header = new SoapHeader('http://192.168.0.170/TT/BookingAPI', 'AuthenticationData', $headerpara, false);
        $client = new SoapClient($wsdl, array('trace' => 1, "cache_wsdl" => WSDL_CACHE_DISK));
        ...
        foreach($header['se']['SearchResult']->Result->WSResult as $res) {
            ...
            if (!is_array($res->Segment->WSSegment)) {
                ...
                $travel[$i]->origin = $res->Segment->WSSegment->Origin->CityCode;
                $travel[$i]->destination = $res->Segment->WSSegment->Destination->CityCode;
                ...
                $hrs_arr = intval($arrival[0]);
                $mins_arr = intval($arrival[1]);
                $temp[$i]->to_hidden = ($hrs_arr * 60) + $mins_arr;
                $depFilterArr = ($hrs_arr * 60) + $mins_arr;
                if($depFilterArr > $max_durationArr){
                    $max_durationArr = $depFilterArr;
                }
                if($depFilterArr < $min_durationArr){
                    $min_durationArr = $depFilterArr;
                }
                ...
                if(isset($_SESSION['IsDomestic']) && ($_SESSION['IsDomestic'] == 0)){
                    foreach($res->Segment->WSSegment as $wssegment){
                        if($wssegment->SegmentIndicator == 1 && ($wssegment->Destination->AirportCode == $data['data']['to'] || $wssegment->Destination->CityCode == $data['data']['to'])){
                            $arrive_outbound = explode("T", $wssegment->ArrTime);
                            $a_outbound = explode(":", $arrive_outbound[1]);
                        }
                        if($wssegment->SegmentIndicator == 2 && ($wssegment->Origin->AirportCode == $data['data']['to'] || $wssegment->Origin->CityCode == $data['data']['to'])){
                            $depart_inbound = explode("T", $wssegment->DepTIme);
                            $d_inbound = explode(":", $depart_inbound[1]);
                        }
                    }
					
                    $temp[$i]->multiple_carriers = 0;
					
					$offset = $this->get_timezone_offset($data['data']['utf_to'], $data['data']['utf_from'])/3600;

                    $durationCorrectionHrs = (int)$offset;
                    $durationCorrectionMins = ($offset - $durationCorrectionHrs) * 60;

                    $depart_outbound = explode("T", $res->Segment->WSSegment[0]->DepTIme);
                    $d_outbound = explode(":", $depart_outbound[1]);
                    $arrive_inbound = explode("T", $res->Segment->WSSegment[count($res->Segment->WSSegment) - 1]->ArrTime);
                    $a_inbound = explode(":", $arrive_inbound[1]);
                    $date_d_out = new DateTime($depart_outbound[0] . ' ' . $depart_outbound[1]);
                    $date_a_out = new DateTime($arrive_outbound[0] . ' ' . $arrive_outbound[1]);
                    $date_d_in = new DateTime($depart_inbound[0] . ' ' . $depart_inbound[1]);
                    $date_a_in = new DateTime($arrive_inbound[0] . ' ' . $arrive_inbound[1]);
                    $diff_out = $date_d_out->diff($date_a_out);
                    $diff_out->i += $durationCorrectionMins;
                    if( $diff_out->i >= 60 ){
                        $diff_out->h = $diff_out->h + $durationCorrectionHrs + 1;
                        $diff_out->i -= 60;
                    }else{
                        $diff_out->h = $diff_out->h + $durationCorrectionHrs;
                    }
                    $day_out = $diff_out->d . 'd';
                    $hr_out = $diff_out->h . 'h';
                    $min_out = $diff_out->i . 'm';
					$total_min_out = $day_out * 24 * 60;
                    $total_min_out += $hr_out * 60;
                    $total_min_out += $min_out;

                    $day_out = floor ($total_min_out / 1440);
                    if ($day_out > 0){
                        $day_out = $day_out . 'd';
                    }
                    else{
                    ...
                    $hr_out = floor (($total_min_out - $day_out * 1440) / 60) . 'h';
                    $min_out = $total_min_out - ($day_out * 1440) - ($hr_out * 60) . 'm';

                    $offset = $this->get_timezone_offset($data['data']['utf_from'], $data['data']['utf_to'])/3600;

                    $durationCorrectionHrs = (int)$offset;
                    $durationCorrectionMins = ($offset - $durationCorrectionHrs) * 60;
                    
                    $diff_in = $date_d_in->diff($date_a_in);
                    $diff_in->i += $durationCorrectionMins;
                    if( $diff_in->i >= 60 ){
                        $diff_in->h = $diff_in->h + $durationCorrectionHrs + 1;
                        $diff_in->i -= 60;
                    }else{
                        $diff_in->h = $diff_in->h + $durationCorrectionHrs;
                    }
                    $day_in = $diff_in->d;
                    $hr_in = $diff_in->h;
                    $min_in = $diff_in->i;
                    $total_min_in = $day_in * 24 * 60;
                    $total_min_in += $hr_in * 60;
                    $total_min_in += $min_in;
					
                    $day_in = floor($total_min_in / 1440);
					if ($day_in > 0){
						$day_in = $day_in . 'd';
					}
					else{
                    ...
                    $hr_in = floor (($total_min_in - $day_in * 1440) / 60) . 'h';
                    $min_in = $total_min_in - ($day_in * 1440) - ($hr_in * 60) . 'm';
                }
                ...
                if( $_SESSION['IsDomestic'] == 0 ){
                    $date_d = new DateTime($flights_search["Search"]["request"]["DepartureDate"]);
                    $date_a = new DateTime($flights_search["Search"]["request"]["ReturnDate"]);
                    $diff = $date_d->diff($date_a);
                    $tday = $diff->d;
                    $thr = $diff->h . 'h';
                    $tmin = $diff->i . 'm';
                ...
                $travel_connecting[$i]->origin = $res->Segment->WSSegment[0]->Origin->CityCode;
                if( isset($_SESSION['IsDomestic']) && $_SESSION['IsDomestic'] == 1 ){
                    $travel_connecting[$i]->destination = $res->Segment->WSSegment[count($res->Segment->WSSegment) - 1]->Destination->CityCode;
                }else{
                    $travel_connecting[$i]->destination = $data['data']['to'];
                }
                $hrsCon = intval($d[0]);
                $minsCon = intval($d[1]);
                ...
                $temp[$i]->to = $a[0] . ':' . $a[1];
                $hrsConArr = intval($a[0]);
                $minsConArr = intval($a[1]);
                $temp[$i]->to_hidden = ($hrsConArr * 60) + $minsConArr;
                $depFilterArr = ($hrsConArr * 60) + $minsConArr;
                if ($depFilterArr > $max_durationArr) {
                    $max_durationArr = $depFilterArr;
                }
                ...
                if(isset($_SESSION['IsDomestic']) && $_SESSION['IsDomestic'] == 0){
                    if ($day_out == 0) $temp[$i]->duration_out = $hr_out . ' ' . $min_out;
                    else $temp[$i]->duration_out = $day_out . ' ' . $hr_out . ' ' . $min_out;
                    if ($day_in == 0) $temp[$i]->duration_in = $hr_in . ' ' . $min_in;
                    else $temp[$i]->duration_in = $day_in . ' ' . $hr_in . ' ' . $min_in;
                }
                $temp[$i]->fare = $total_fare;
                ...
                foreach($res->Segment->WSSegment as $connectingFlight) {
                    ...
                    $travel_connecting[$i]->connecting[$j]->origin = $connectingFlight->Origin->AirportCode;
                    $travel_connecting[$i]->connecting[$j]->origin_city = $connectingFlight->Origin->CityCode;
                    $travel_connecting[$i]->connecting[$j]->destination = $connectingFlight->Destination->AirportCode;
                    $travel_connecting[$i]->connecting[$j]->destination_city = $connectingFlight->Destination->CityCode;
                    $temp[$i]->multi[$j]->origin = $travel_connecting[$i]->connecting[$j]->origin;
                    $temp[$i]->multi[$j]->destination = $travel_connecting[$i]->connecting[$j]->destination;
                    $temp[$i]->multi[$j]->origin_city = $travel_connecting[$i]->connecting[$j]->origin_city;
                    $temp[$i]->multi[$j]->destination_city = $travel_connecting[$i]->connecting[$j]->destination_city;
        ...
        $resultArray['wsresult'] = (array)$header['se']['SearchResult']->Result->WSResult;
        $resultArray['indResults'] = array(
            ...
            'max_durationArr' => $max_durationArr,
            'min_durationArr' => $min_durationArr
        );
    ...
    function return_parameters()
    {
        ...
        $searchStr = "India";
        $isInternational = 0;
        if ( !is_numeric(strrpos($data['from'], $searchStr)) || !is_numeric(strrpos($data['to'], $searchStr)) ) {
            $isInternational = 1;
        }
        ...
        if( $isInternational == 1 ){
            ...
            $_GET['total_count'] = $_GET['adult_count']+$_GET['youth_count']+$_GET['kids_count'];
            $this->load->view("common/header.php");
            $this->load->view('flights_view/display_flights_international_return.php');
            $this->load->view("common/footer.php");
        }else{
            $_SESSION['currentUrlFlight'] = current_full_url();
            $_GET['total_count'] = $_GET['adult_count']+$_GET['youth_count']+$_GET['kids_count'];
    ...
    function test_multi()
    {
        ...
        if ($chk_edit != 'true') {
            if ((isset($_SESSION['cnt_val']) && $_SESSION['cnt_val'] > 1) && $is_mod != 'true') {
                ...
                if(!empty($_GET['city_to_three']) && !empty($_GET['city_to_four'])){
                    $total_flights = 4;
                }
                if(!empty($_GET['city_to_three']) && (empty($_GET['city_to_four'])) ){
                    $total_flights = 3;
                ...
                for ($i = 0; $i < $total_flights; $i++) {
                    ...
                    $utf_from[$i] = $this->input->get('utf_from_' . $count_num[$i]);
                    $utf_to[$i] = $this->input->get('utf_to_' . $count_num[$i]);
                ...
                redirect('api/flights/set_multi_url?city_from=' . $_SESSION['details']['from'][0] . '&city_to=' . $_SESSION['details']['to'][0] . '&utf_from=' . $_SESSION['details']['utf_from'][0] . '&utf_to=' . $_SESSION['details']['utf_to'][0] . '&adult_count=' . $_SESSION['details']['adult_count'] . '&youth_count=' . $_SESSION['details']['youth_count'] . '&kids_count=' . $_SESSION['details']['kids_count'] . '&multi_date=' . $_SESSION['details']['dates'][0] . '&flight_num=' . $_SESSION['cnt_val'] . '&airline_preference_3=' . $_SESSION['airline_preference'] . '&class_of_travel_3=' . $_SESSION['class_travel']);
            } 
        } else { 
            if( isset($_SESSION['currentUrlBus']) && (isset($_SESSION['ovMode'])) && ($_SESSION['ovMode']->mode == 'bus') ){
                ...
                redirect('api/flights/set_multi_url?city_from=' . $_SESSION['details']['from'][$i] . '&city_to=' . $_SESSION['details']['to'][$i] . '&utf_from=' . $_SESSION['details']['utf_from'][$i] . '&utf_to=' . $_SESSION['details']['utf_to'][$i] . '&adult_count=' . $_SESSION['details']['adult_count'] . '&youth_count=' . $_SESSION['details']['youth_count'] . '&kids_count=' . $_SESSION['details']['kids_count'] . '&multi_date=' . $_SESSION['details']['dates'][$i] . '&flight_num=' . $_SESSION['cnt_val'] . '&airline_preference_3=' . $_SESSION['airline_preference'] . '&class_of_travel_3=' . $_SESSION['class_travel']);
            }
        }
    }
    ...
    function set_multi_url()
    {
        ...
        $_SESSION['details'] = array(
            ...
            'utf_from' => $_SESSION['details']['utf_from'],
            'utf_to' => $_SESSION['details']['utf_to'],
    ...
    function sample()
    {
		...
		$overview_data['data']->booking_details = $data['booking_details'];
    ...
    {
        $data = $this->input->post(null, true);
		$_SESSION['multi_domestic_check'] = 1;

        $utf_from = $data['data']['utf_from'];
        $utf_to = $data['data']['utf_to'];

        $durationCorrection = $this->get_timezone_offset($utf_to, $utf_from) / 3600;
		...
		$_SESSION['multi_isDomestic'][$_SESSION['cnt_val'] - 1] = $_SESSION['IsDomestic'];
        ...
        $_SESSION['search_sessionid'] = $header['se']['SearchResult']->SessionId;
        ...
        foreach($header['se']['SearchResult']->Result->WSResult as $res) {
            ...
            if (!is_array($res->Segment->WSSegment)) {
                ...
                $date_a = new DateTime($atime[0] . '' . $atime[1]);
                $diff = $date_d->diff($date_a);
                $durationCorrectionHrs = (int)$durationCorrection;
                $durationCorrectionMins = ($durationCorrection - $durationCorrectionHrs) * 60;
                $diff->i += $durationCorrectionMins;
                if( $diff->i >= 60 ){
                    $diff->h = $diff->h + $durationCorrectionHrs + 1;
                    $diff->i -= 60;
                }else{
                    $diff->h = $diff->h + $durationCorrectionHrs;
                }

                $d = $diff->d . 'd';
                $hr = $diff->h . 'h';
                $min = $diff->i . 'm';
				$total_min_in = $d * 24 * 60;
                $total_min_in += $hr * 60;
                $total_min_in += $min;

                $d = floor ($total_min_in / 1440);
				if($d > 0){
					$d = $d . 'd';
				}
				else{
                ...
                $hr = floor (($total_min_in - $d * 1440) / 60) . 'h';
                $min = $total_min_in - ($d * 1440) - ($hr * 60) . 'm';
                ...
                $temp[$i]->to = $a[0] . ':' . $a[1];
                if ($day == 0) $temp[$i]->duration = $hr . ' ' . $min;
                else $temp[$i]->duration = $day . ' ' . $hr . ' ' . $min;
                $temp[$i]->fare = $total_fare;
                if ($temp[$i]->fare > $highest) {
                    $highest = $temp[$i]->fare;
        ...
        if( isset($_SESSION['redir_data']['flight_num']) ){
            $flight_num = intval($_SESSION['redir_data']['flight_num']);
            $_SESSION['multiTravelData']['flightData']['fligh_num'] = $flight_num;
        }

        $resultArray['wsResult'] = $header['se']['SearchResult']->Result->WSResult;
        $resultArray['indResults'] = array(
            'layover' => $temp_layover,
            'results' => $temp,
            'fare_max' => $highest,
    ...
    function chk_lowest_fare(){
        if( isset($_SESSION['multiTravelData']['flightData']) ){
            echo json_encode($_SESSION['multiTravelData']['flightData']);
        }else{
            $this->search_flights_oneway();
        }
    }
    ...
    function traveller_details()
    {
        ...
        if ($data['flight_type'] == 0) {
            $this->load->view("common/header.php");
            $convenience = $this->convenience_model->get_convenience_charge('flights');
    ...
    public function return_international_traveller_details(){
        $_SESSION['calling_controller_name'] = "flights";
        ...
        $data['outbound_source'] = $data['outbound_booking_details']->rest->Origin;
        $data['outbound_destination'] = $data['outbound_booking_details']->rest->Destination;
        ...
        if ($data['outbound_flight_type']->type) {
            ...
            $GFQResult = $this->get_fare_quote_return($data['outbound_booking_details'], $data['type_flight']);
            $_SESSION['outboundGFQResult'] = $GFQResult;
            $_SESSION['InboundFare'] = $_SESSION['ReturnFareQuote'];
        ...
        $data['convenience_charge_msg'] = $convenience->convenience_charge_msg;
        $this->load->view("common/header.php");
        $this->load->view('flights/traveller_internation_return_book_page', array(
            'data' => $data
        ));
        $this->load->view("common/footer.php");
    }
    ...
    function return_traveller_details()
    {
        ...
        if ($data['inbound_flight_type']->type) {
            ...
            $GFQResult = $this->get_fare_quote_return($data['inbound_booking_details'], $data['type_flight']);
            $_SESSION['inboundGFQResult'] = $GFQResult;
            $_SESSION['InboundFare'] = $_SESSION['ReturnFareQuote'];
        }
        $this->load->model('admin/convenience_model');
        $convenience = $this->convenience_model->get_convenience_charge('flights');
    ...
    {
		for($i=0;$i<count($_SESSION['flight_data']);$i++){
            if($_SESSION['multi_isDomestic'][$i] == 0){
                $_SESSION['multi_domestic_check'] = 0;
                break;
            }
        }
        $this->load->model('admin/convenience_model');
    ...
    function get_fare_quote($data)
    {
		...
		if( $header['se']['GetFareQuoteResult']->Status->StatusCode == 02 && $header['se']['GetFareQuoteResult']->Status->Description == "The Fare quotes were unsucessfull" ){
        ...
        $data['booking_details'] = new StdClass;
        $data['booking_details']->rest = $header['se']['GetFareQuoteResult']->Result;
        $_SESSION['GFQFareDetails'] = $data;
        return $data;
    }

    function get_fare_quote_return($data, $inout)
    {

        $temp['booking_details'] = $data;
        unset($data);
        $data['booking_details'] = $temp['booking_details'];
        $data['type_flight'] = $inout;
            ...
            $fare["GetFareQuote"]["fareQuoteRequest"]["Result"]['WSResult']["Fare"] = (array)$data['booking_details']->rest->Fare;
            $fare["GetFareQuote"]["fareQuoteRequest"]["Result"]['WSResult']["FareBreakdown"]["WSPTCFare"] = (array)$data['booking_details']->rest->FareBreakdown->WSPTCFare;
            ...
            $jsonData = json_encode($data['booking_details']);
            $outData = json_decode($jsonData, true);
            $fare["GetFareQuote"]["fareQuoteRequest"]["Result"]['WSResult']["Segment"]["WSSegment"] = $outData['rest']['Segment']['WSSegment'];
            ...
            if ($data['type_flight'] == "outbound") {
                ...
                $total_tax_fare_out_before = $fare_arr_out_before['AdditionalTxnFee'] + $fare_arr_out_before['AirTransFee'] + $fare_arr_out_before['Tax'] + $fare_arr_out_before['OtherCharges'] + $fare_arr_out_before['ServiceTax'];
            ...
            $GFQResult = $header['se'];
            if( !isset($header['se']['GetFareQuoteResult']->Result) ){
                redirect('api/flights/booking_failed');
            }

            $fare_obj = $header['se']['GetFareQuoteResult']->Result->Fare;
            $_SESSION['ReturnFareQuote'] = $header['se']['GetFareQuoteResult']->Result->Fare;
            if ($data['type_flight'] == "outbound") {  
                ...
                $data['total_fare_field'] = $total_fare_after_out;
                ...
                $data['total_fare_field'] = $total_fare_after_in;
    ...
    function transactionFail(){
        ...
        $this->load->view('flights/transactionFail');
    ...
    function book()
    {
        ...
        */
        ...
        $fare = $_SESSION['flight_db_fare_params']["flight_total_Fare"];

        if($fare > $balance){
            redirect('api/flights/transactionFail');
        }
        //get agency balance end//
        
        if( isset($_SESSION['fareQuote']) ){
            $data['fare'] = (array)$_SESSION['fareQuote']->Fare;
        }else{
            $data['fare'] = json_decode(stripslashes($data['fare']), TRUE);
        }
        ...
        if( !empty($passengerData['adult_title_csv']) )        
        {
            $passenger['adult_title_csv'] = implode(',', $passengerData['adult_title_csv']);
        }

        if( !empty($passengerData['adult_first_name_csv']) )                
        {
            $passenger['adult_first_name_csv'] = implode(',', $passengerData['adult_first_name_csv']);
        }

        if( !empty($passengerData['adult_last_name_csv']) )                
        {
            $passenger['adult_last_name_csv'] = implode(',', $passengerData['adult_last_name_csv']);
        }

        if( !empty($passengerData['pass_number_a']) )                
        {
            $passenger['pass_number_a'] = implode(',', $passengerData['pass_number_a']);
        }

        if( !empty($passengerData['pass_expiry_a']) )                
        {
            $passenger['pass_expiry_a'] = implode(',', $passengerData['pass_expiry_a']);
        }

        if( !empty($passengerData['kid_title_csv']) )        
        {
            $passenger['kid_title_csv'] = implode(',', $passengerData['kid_title_csv']);
        }

        if( !empty($passengerData['kid_first_name_csv']) )                
        {
            $passenger['kid_first_name_csv'] = implode(',', $passengerData['kid_first_name_csv']);
        }

        if( !empty($passengerData['kid_last_name_csv']) )                
        {
            $passenger['kid_last_name_csv'] = implode(',', $passengerData['kid_last_name_csv']);
        }

        if( !empty($passengerData['kid_dob_csv']) )                
        {
            $passenger['kid_dob_csv'] = implode(',', $passengerData['kid_dob_csv']);
        }

        if( !empty($passengerData['pass_number_k']) )                
        {
            $passenger['pass_number_k'] = implode(',', $passengerData['pass_number_k']);
        }

        if( !empty($passengerData['pass_expiry_k']) )                
        {
            $passenger['pass_expiry_k'] = implode(',', $passengerData['pass_expiry_k']);
        }


        if( !empty($passengerData['infant_title_csv']) )        
        {
            $passenger['infant_title_csv'] = implode(',', $passengerData['infant_title_csv']);
        }

        if( !empty($passengerData['infant_first_name_csv']) )                
        {
            $passenger['infant_first_name_csv'] = implode(',', $passengerData['infant_first_name_csv']);
        }

        if( !empty($passengerData['infant_last_name_csv']) )                
        {
            $passenger['infant_last_name_csv'] = implode(',', $passengerData['infant_last_name_csv']);
        }

        if( !empty($passengerData['infant_dob_csv']) )                
        {
            $passenger['infant_dob_csv'] = implode(',', $passengerData['infant_dob_csv']);
        }

        if( !empty($passengerData['pass_number_i']) )                
        {
            $passenger['pass_number_i'] = implode(',', $passengerData['pass_number_i']);
        }

        if( !empty($passengerData['pass_expiry_i']) )                
        {
            $passenger['pass_expiry_i'] = implode(',', $passengerData['pass_expiry_i']);
        }
        ...
        if ($data['flight_type'] == 0) {
            ...
            if( isset($data['fbBookingId']) ){
                $_SESSION['fb_bookingId_flights'] = $data['fbBookingId'];
            }
        ...
        if ($data['flight_type'] == 1) {
            ...
            $fbBookingId = $fbBooking . $randomNum;
            $data['fbBookingId'] = $fbBookingId;
            $_SESSION['fb_bookingId_flights'] = $fbBookingId;
    ...
    function book_return_international()
    {
        $data = $this->input->post(null, true);
        ...
        */
        ...
        if( !empty($passengerData['title_a']) )        
        {
            $passenger['adult_title_csv'] = implode(',', $passengerData['title_a']);
        }

        if( !empty($passengerData['first_name_a']) )                
        {
            $passenger['adult_first_name_csv'] = implode(',', $passengerData['first_name_a']);
        }

        if( !empty($passengerData['last_name_a']) )                
        {
            $passenger['adult_last_name_csv'] = implode(',', $passengerData['last_name_a']);
        }

        if( !empty($passengerData['pass_number']) )                
        {
            $passenger['pass_number'] = $passengerData['pass_number'];
        }

        if( !empty($passengerData['pass_expiry']) )                
        {
            $passenger['pass_expiry'] = $passengerData['pass_expiry'];
        }
        ...
        if( !empty($passengerData['title_k']) )        
        {
            $passenger['kid_title_csv'] = implode(',', $passengerData['title_k']);
        }

        if( !empty($passengerData['first_name_k']) )                
        {
            $passenger['kid_first_name_csv'] = implode(',', $passengerData['first_name_k']);
        }

        if( !empty($passengerData['last_name_k']) )                
        {
            $passenger['kid_last_name_csv'] = implode(',', $passengerData['last_name_k']);
        }

        if( !empty($passengerData['dob_k']) )                
        {
            $passenger['kid_dob_csv'] = implode(',', $passengerData['dob_k']);
        }
        ...
        if( !empty($passengerData['title_i']) )        
        {
            $passenger['infant_title_csv'] = implode(',', $passengerData['title_i']);
        }

        if( !empty($passengerData['first_name_i']) )                
        {
            $passenger['infant_first_name_csv'] = implode(',', $passengerData['first_name_i']);
        }

        if( !empty($passengerData['last_name_i']) )                
        {
            $passenger['infant_last_name_csv'] = implode(',', $passengerData['last_name_i']);
        }

        if( !empty($passengerData['dob_i']) )                
        {
            $passenger['infant_dob_csv'] = implode(',', $passengerData['dob_i']);
        }
        ...
        if ($data['details']->outbound_flight_type->type == 0) {
            ...
            */
            ...
            $book = $getReturnBookRequestObj->setReturnBookRequest($data, $passengerData);
            $result = $flightsSOAPObj->makeSOAPCall("Book", $book);
            ...
            if ($l == 1) {
                ...
                if ($data['details']->inbound_flight_type->type == 0) {
                    ...
                    */
                    ...
                    $book = $getInboundBookRequestObj->setInboundBookRequest($data, $passengerData);
                    $result = $flightsSOAPObj->makeSOAPCall("Book", $book);
    ...
    function internation_return_ticket(){

        $ticketid = $_SESSION['outbound'];
        $fareQuoteFlag = 1;

        if( isset($_SESSION['outboundGFQResult']) ){
            $GFQFareDetails = $_SESSION['outboundGFQResult']; 
            $fareQuoteFlag = 1;

            $GFQFare = json_encode($GFQFareDetails['GetFareQuoteResult']->Result->Fare);
            $GFQFareBreakdown = json_encode($GFQFareDetails['GetFareQuoteResult']->Result->FareBreakdown);
            $GFQFareRule = json_encode($GFQFareDetails['GetFareQuoteResult']->Result->FareRule);

            $GFQFare = json_decode($GFQFare ,TRUE);
            $GFQFareBreakdown = json_decode($GFQFareBreakdown ,TRUE);
            $GFQFareRule = json_decode($GFQFareRule ,TRUE);
        }
        ...
        if( $fareQuoteFlag ){
            ...
            $data['booking_details'] = $data['rest']['Segment']['WSSegment'];
            $data['fare'] = $GFQFare;
            $data['fare_breakdown'] = $GFQFareBreakdown;
            $data['fare_rule'] = $GFQFareRule;
            $data['origin'] = $data['rest']['Origin'];
            $data['destination'] = $data['rest']['Destination'];
            $data['Source'] = $data['rest']['Source'];
            $data['return_session_id'] = $_SESSION['outbound_id'];
        ...
        $ticket = $getTicketDetailsObj->setTicketDetails($data, $details, $passengerDetails);
        ...
        if ($result->TicketResult->PNR != "") {
            ...
            $ticket_bookingid = $this->flight_model->setTicketDetails($info);
        }
    ...
    function ticket_details($ticketid, $guest_email = null)
    {
        ...
        if( !empty($guest_email) ){
            ...
            $data[$i] = $this->flight_model->fetchTicketDetailsByBookingId($ticketid);
        } else {
        ...
        if( isset($_SESSION['myTicketChk']) ){
            ...
            foreach( $data as $d ){
                ...
                $this->send_email($d[0]->fbBooking_id, $userEmail, $userFirstName, $additionalInfo);
				}
            $this->load->view('common/header.php');
            $this->load->view('flights/multiway_tickets.php', array(
                'data' => $data
    ...
    function book_assorted(){
		...
		unset($_SESSION['internationalFlightExists']);
        ...
        for ($i = 0; $i < $_SESSION['details']['total_count']; $i++) {
            ...
            if( $i == 0 ){
                ...
                $data['lead_adult_dob'] = $data['dob' . $i . ''];
            }else{
                //others
                ...
                if( $traveller_type == 'adult' ){
                    ...
                    if( isset($data['pass_expiry' . $i . '']) ){
                        $data['pass_expiry_a'][] = $data['pass_expiry' . $i . ''];
                        $data['pass_number_a'][] = $data['pass_number' . $i . ''];
                    }
                    ...
                    if( isset($data['pass_expiry' . $i . '']) ){
                        $data['pass_expiry_k'][] = $data['pass_expiry' . $i . ''];
                        $data['pass_number_k'][] = $data['pass_number' . $i . ''];
                    }
                    ...
                    if( isset($data['pass_expiry' . $i . '']) ){
                        $data['pass_expiry_i'][] = $data['pass_expiry' . $i . ''];
                        $data['pass_number_i'][] = $data['pass_number' . $i . ''];
                    }
                }else{
        ...
        $data['details'][$m] = (Object)$data['details'][$m];
            ...
            if ($data['details'][$m]->flight_type->type == 0) {
                ...
                $fare = $fare = $data['details'][$m]->ov->total_fare_field;
                $fare = str_replace(',', '', $fare);
                ...
                if ($data['details'][$m]->adult_count_field >= 1) {
                    ...
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["DateOfBirth"] = date('c',strtotime($data['lead_adult_dob']));
                    ...
                    if( isset($_SESSION['IsDomestic']) && $_SESSION['IsDomestic'] == 0 ){
                        $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportNumber"] = $data['pass_number0'];
                        $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportExpiry"] = date('c',strtotime($data['pass_expiry0']));
                    }else{
                ...
                while ($i < $data['details'][$m]->adult_count_field) {
                    ...
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Type"] = "Adult";
                    if( isset($data['dob_a'][$j]) && !empty($data['dob_a'][$j]) ){
                    ...
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportNumber"] = $data['pass_number_a'][$j];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportExpiry"] = date('c',strtotime($data['pass_expiry_a'][$j]));
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Pincode"] = "";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Country"] = "IN";
                ...
                /*....*/
                $j = 0;
                while ($j < $data['details'][$m]->youth_count_field) {
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Title"] = $data['title_k'][$j];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["FirstName"] = $data['first_name_k'][$j];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["LastName"] = $data['last_name_k'][$j];
                    ...
                    if ($data['title_k'][$j] == 'Mr' || $data['title_k'][$j] == 'Master') {
                        $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Gender"] = "Male";
                    }
                    else {
                        $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Gender"] = "Female";
                    }
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportNumber"] = $data['pass_number_k'][$j];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportExpiry"] = date('c',strtotime($data['pass_expiry_k'][$j]));
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Pincode"] = "";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Country"] = "IN";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Phone"] = $data['lead_adult_mobile_no'];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["AddressLine1"] = "#37, 2nd cross, s.k.garden, bangalore - 560043";
                ...
                /*--*/
                $j = 0;
                while ($j < $data['details'][$m]->kids_count_field) {
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Title"] = $data['title_i'][$j];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["FirstName"] = $data['first_name_i'][$j];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["LastName"] = $data['last_name_i'][$j];
                    ...
                    if ($data['title_i'][$j] == 'Mr' || $data['title_i'][$j] == 'Master') {
                        $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Gender"] = "Male";
                    }
                    else {
                        $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Gender"] = "Female";
                    }
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportNumber"] = $data['pass_number_i'][$j];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportExpiry"] = date('c',strtotime($data['pass_expiry_i'][$j]));
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Pincode"] = "";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Country"] = "IN";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Phone"] = $data['lead_adult_mobile_no'];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["AddressLine1"] = "#37, 2nd cross, s.k.garden, bangalore - 560043";
                ...
                if (!is_array($data['details'][$m]->booking_details->rest->FareRule->WSFareRule)) {
                    ...
                    if( isset($data['details'][$m]->booking_details->rest->FareRule->WSFareRule->FareRestriction) ){
                ...
                if( isset($_SESSION['sess_id']) && count($_SESSION['sess_id']) > 1 ){
                    $book["Book"]["bookRequest"]["SessionId"] = $_SESSION['sess_id'][$m];
                }else{
                    $book["Book"]["bookRequest"]["SessionId"] = $_SESSION['sess_id'][key($_SESSION['sess_id'])];
                }
                ...
                if ($header['se']['BookResult']->Status->Description == "Successful" || $header['se']['BookResult']->Status->Description == "Fare is not available at the time of booking") {
                    ...
                    if( isset($_SESSION["user_details"][0]) ){
                        $setData['user_id'] = $_SESSION['user_details'][0]->user_id;
                    }else{
                        $setData['user_id'] = $_SESSION['user_details']['user_id'];
                    }
    ...
    function book_multi()
    {
		...
		$total = 0;
		for( $i=0;$i<$key;$i++){
			$data['details'][$i]->total_fare_field = str_replace(',', '', $data['details'][$i]->total_fare_field);
			$total = $total + $data['details'][$i]->total_fare_field;
		}
        //get agency balance//
        /**
        * Object initialisation
        ...
        */
        ...
        for ($m = 0; $m < count($_SESSION['flight_data']); $m++) {
            ...
            if ($data['details'][$m]->flight_type->type == 0) {
                ...
                if ($data['details'][$m]->adult_count_field >= 1) {
                    ...
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportExpiry"] = date('c',strtotime($data['pass_expiry']));
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Pincode"] = "";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Country"] = "IN";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Phone"] = $data['lead_adult_mobile_no'];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["AddressLine1"] = "#37, 2nd cross, s.k.garden, bangalore - 560043";
                ...
                while ($j < $data['details'][$m]->youth_count_field) {
                    ...
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportNumber"] = $data['pass_number'][$j];
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["PassportExpiry"] = date('c',strtotime($data['pass_expiry_k'][$j]));
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Pincode"] = "";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Country"] = "IN";
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Phone"] = $data['lead_adult_mobile_no'];
                ...
                while ($j < $data['details'][$m]->kids_count_field) {
                    ...
                    $book["Book"]["bookRequest"]["Passenger"]["WSPassenger"][$i]["Fare"]["ReverseHandlingCharge"] = $data['details'][$m]->rest->Fare->ReverseHandlingCharge / $data['details'][$m]->kids_count_field;
                ...
                if (!is_array($data['details'][$m]->rest->FareRule->WSFareRule)) {
                    ...
                    if( isset($data['details'][$m]->rest->FareRule->WSFareRule->FareRestriction) ){
                    ...
                    for ($t = 0; $t < count($data['details'][$m]->rest->FareRule->WSFareRule); $t++) {
                        ...
                        if( isset($data['details'][$m]->rest->FareRule->WSFareRule[$t]->FareRestriction) ){
        ...
        $_SESSION['multiway_postdata']['key'] = 'LcXB2s';
    ...
    function cancel_ticket_admin()
    {
        ...
        else if($header['se']['CancelItineraryResult']->Status->Description == "Only Non LCC booking can be cancelled"){
            $_SESSION['cancelResult'] = "Only Non LCC booking can be cancelled";
        }
    ...
    function ticket_assorted(){
        ...
        for ($f = 0; $f < count($ticketid); $f++) {
            ...
            if ($header['se']['TicketResult']->PNR != "") {
                ...
                if( isset($_SESSION['flight_db_fare_params']) ){
                    $info['OfferedPrice'] = $_SESSION['flight_db_fare_params']["flight_offered_price"];
                    $info['PublishedPrice'] = $_SESSION['flight_db_fare_params']["flight_published_price"];
                    $info['ConvenienceCharge'] = $_SESSION['flight_db_fare_params']["flight_convenience_charge"];
                    $info['TotalFare'] = $_SESSION['flight_db_fare_params']["flight_total_Fare"];
                }else{
                    $info['OfferedPrice'] = 0;
                    $info['PublishedPrice'] = 0;
                    $info['ConvenienceCharge'] = 0;
    ...
    function ticket()
    {
        ...
        if( isset($_SESSION['GFQFareDetails']) ){
            $GFQFareDetails = $_SESSION['GFQFareDetails']; 
            $fareQuoteFlag = 1;

            $GFQFare = json_encode($GFQFareDetails['booking_details']->rest->Fare);
            $GFQFareBreakdown = json_encode($GFQFareDetails['booking_details']->rest->FareBreakdown);
            $GFQFareRule = json_encode($GFQFareDetails['booking_details']->rest->FareRule);
    ...
    function getSSR($result, $sessionId){
        ...
        $getSSR['GetSSR']['ssrRequest']['Result'] = $result['GetFareQuote']['fareQuoteRequest']['Result']['WSResult'];
        $getSSR['GetSSR']['ssrRequest']['SessionId'] = $sessionId;

        $result = $flightsSOAPObj->makeSOAPCall("GetSSR", $getSSR);

    }

    function international_return_ticket(){
        ...
        if( $fareQuoteFlag ){
            ...
            $data = json_decode($details[0]->extra_info, TRUE);
            $data['booking_details'] = $data['rest']['Segment']['WSSegment'];
            $data['fare'] = $data['rest']['Fare'];
            $data['fare_breakdown'] = $data['rest']['FareBreakdown'];
            $data['fare_rule'] = $data['rest']['FareRule'];
            $data['origin'] = $data['rest']['Origin'];
            $data['destination'] = $data['rest']['Destination'];
            $data['Source'] = $data['rest']['Source'];
            $data['return_session_id'] = $_SESSION['outbound_id'];
    ...
    function guest_details()
    {
        ...
        if ($guest_id) {
            ...
            $_SESSION['invalidGuestErrorMessage'] = '<div class="select-error"><h6 style="text-align:center; font-size:12px; font-weight: 200;">Invalid User ID or Booking ID.</h6></div>';
            $this->guest_ticket();
        }
    }
    ...
    function ticket_page()
    {
        ...
        $this->load->model('flight_model');
        $data = $this->flight_model->fetchTicketDetailsByBookingId($bookingid);
