	...
	class GetTicketInformation{
		...
		public function setTicketInformation($data, $bookingResult){
            ...
            if(is_array($bookingResult->GetBookingResult->Ticket->WSTicket)){
                foreach ( $bookingResult->GetBookingResult->Ticket->WSTicket as $wsticket){
                    if( is_array($wsticket->SegmentAdditionalInfo->WSSegAdditionalInfo) ){
                        foreach( $wsticket->SegmentAdditionalInfo->WSSegAdditionalInfo as $sai ){
                            $bagCsv[] = $sai->Baggage;
                        }
            ...
            if (is_array($bookingResult->GetBookingResult->Segment->WSSegment)) {
                $info['DepTime'] = $bookingResult->GetBookingResult->Segment->WSSegment[0]->DepTIme;
                $info['ArrTime'] = $bookingResult->GetBookingResult->Segment->WSSegment[0]->ArrTime;
                $r = 0;
                ...
                $info['DepTime'] = $bookingResult->GetBookingResult->Segment->WSSegment->DepTIme;
                $info['ArrTime'] = $bookingResult->GetBookingResult->Segment->WSSegment->ArrTime;
