...

{
    ...
    private
    function requestData($latitude, $longitude, $units, $language = 'en', $timestamp = false, $exclusions = false)
    {
        $validUnits = array(
            'auto',
            'us',
        ...
        if (in_array($units, $validUnits)) {
            $request_url = self::API_ENDPOINT . $this->api_key . '/' . $latitude . ',' . $longitude . '?units=' . $units . '&lang=' . $language . ($timestamp ? '&time=' . $timestamp : '') . ($exclusions ? '&exclude=' . $exclusions : '');
            /**
...

{
    ...
    {
        return $this->raw_data->apparentTemperatureMin;
    }
    ...
    {
        return $this->raw_data->apparentTemperatureMax;
    }
