<?php 
    if(isset($_SESSION['checkTime'])){
        $checkTime = $_SESSION['checkTime'];
    }
    $checkTimeEx = explode('|',$checkTime);
    if(count($checkTimeEx)==1){
        $checkTime = "No information available. Please contact the hotel for further assistance.";
    }
?>
...
<?php 
    ...
    $hotel_extra_info = json_decode($data['hotel_extra_info'], false);
    $hotel_info = json_decode($data['hotel_info'], false);
    $room_details = json_decode($data['room_details'], false);
    $hotel_price = 0;
    $hotel_tax = 0;

    foreach( $_SESSION['hotelBlockRoomRequest']['HotelRoomsDetails'] as $hrd ){
        $hotel_price += $hrd['Price']['PublishedPriceRoundedOff'];
        $hotel_tax += $hrd['Price']['Tax'];
    }

    $offered_price = $hotel_info->Price->OfferedPrice;
    ...
    $convenience_charge = ($data['convenience_charge']->convenience_charge/100) * $hotel_price + 100;
    $_SESSION['hotel_conv_charge'] = $convenience_charge;
	$hotel_price_per_day = ($hotel_price - $hotel_tax) / $stay_days;
	$price_without_tax = $hotel_price - $hotel_tax;
    $hotel_price_total = $hotel_price;
    if( !isset($_SESSION['hotel_price_single']) ){
        $_SESSION['hotel_price_single'] = 0;
    }
    $total_hotel_fare = $convenience_charge + $hotel_price_total;
    $_SESSION['hotel_price_single_final'] = $total_hotel_fare;    
?>
<style>
	.checkDate{
        font-weight: bold;
    ...
    <div id="popoverHiddenContent" style="display: none">
        <div class="row center-align-text fl_row">
            ...
            <div class="col-lg-6 col-lg-offset-3 center-align-text">
                <div class="row date-text center-align-text popover-total-fare" style="font-size:18px; padding-top: 1px;"> <i class="fa fa-inr"></i>&nbsp;<span style="font-family:oswald;font-size:18px;"><?php echo $total_hotel_fare;?></span></div>
            </div>
            <hr/>
            <div class="checkTime left-text">
                <span class="h5 heading">Check-In Check-Out Time</span>
                    <div class="row check left-text">
                    <?php $checkTimes = explode('|',$checkTime);
                        $count = count($checkTimes);
                        if($count == 1){?>
                            <ul>
                                <li><?php echo $checkTime;?></li>
                            </ul>
                        <?php } 
                        else{
                            for ($i=0;$i<$count-1;$i++) {?>
                                <ul>
                                    <li><?php echo $checkTimes[$i];?></li>
                                </ul>
                            <?php }
                        }?>
    ...
    <div id="popoverHiddenTitle" style="display: none">
        <div class="row">
            <div class="col-lg-24">
				...
				<div class="col-lg-8 pop-title-text">Check-In: <span class="checkDate"><?php echo date("d M Y",strtotime($data['checkin_time']));?></span></div>
                <div class="col-lg-8 pop-title-text">Check-Out: <span class="checkDate"><?php echo date("d M Y",strtotime($data['checkout_time']));?></span></div>
            </div>
        </div>
    </div>
</div> 
                ...
                <div class="row fl_overwiew">
                    <div class="col-lg-5 fl_septr1">
                        ...
                        <div class="col-lg-14 fl_info">
                            <div class="row">
                                <div class="col-lg-24 center-align-text travel-text">
                                    <div class="row center-align-text">
                                        <div class="col-lg-offset-2 col-lg-23 remove-padding srcDest" id="origin" style="font-size:14px;"><?php echo $destination[0].', '.$iso[count($iso)-2];?></div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-lg-23 col-lg-offset-1 center-align-text TotFare1 fare-padding"> <i class="fa fa-inr"></i>&nbsp;<span><?php echo $total_hotel_fare;?></span></div>
                            </div>
                        </div>
        ...
        <div class="container">
            <div class="col-xs-24 col-sm-18 col-md-18" id="error_msg">
                <form action="<?php echo site_url('new_request/payment_gateway')?>" method="post" id="travellers-form" enctype="multipart/form-data">
					...
					<input name="guest_email" value="" style="display:none;" />
					<input name="guest_phone" value="" style="display:none;" />
                </form>
            </div>
                    ...
                    <div class="col-xs-24 col-sm-24 col-md-24 bookingSelectionDetails">
                        <table class="table table-condensed table-custom" >
                            <tbody>
                                ...
                                <tr>
                                    <td>Base Fare </td>
                                    <td class="right-text tf-cng discount-value"><i class="fa fa-inr"></i>&nbsp; <?php echo number_format($price_without_tax);?> (<i class="fa fa-inr"></i>&nbsp; <?php echo number_format($hotel_price_per_day);?> /night)</td>
                                </tr>
                                <tr>
                                    <td>Taxes</td>
                                    <td class="right-text tf-cng"><i class="fa fa-inr"></i>&nbsp; <?php echo number_format($hotel_tax);?></td>
                                </tr>
                                <tr>
                                    <td>Discount</td>
                                ...
                                <tr>
                                    ...
                                    <td class="right-text tf-cng discount-value"><i class="fa fa-inr"></i>&nbsp; <?php echo number_format((float)$convenience_charge,2);?></td>
                                </tr>
                                <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
                                <tr>
