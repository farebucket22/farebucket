...
for($i=0;$i<$c;$i++){
    $iso[$i] = explode(",",$_SESSION['hotel_data'][$i]['search-string-multi']);
    $destination[$i] = explode(",",$_SESSION['hotel_data'][$i]['typed-string-multi']);
    $check_in_time[$i] = $_SESSION['hotel_data'][$i]['checkin_time'];
    $check_out_time[$i] = $_SESSION['hotel_data'][$i]['checkout_time'];
    $checkin_time[$i] = str_replace("/","-",$check_in_time[$i]);
    $checkin_time[$i] = str_replace("/","-",$check_in_time[$i]);
    $checkout_time[$i] = str_replace("/","-",$check_out_time[$i]);
    $diff=date_diff(date_create($checkin_time[$i]),date_create($checkout_time[$i]));
    $stay_days[] = $diff->format("%a");
    $fare_single[$i] = $_SESSION['hotel_data'][$i]['hotel_info']->Price->OfferedPrice;
    $fare_total[$i] = $fare_single[$i]*$stay_days[$i];
    $fare_total[$i] = $fare_single[$i]*$stay_days[$i];
}
    ...
    <?php for($j=0; $j < $c; $j++): ?>
        ...
        <div class="row center-align-text fl_row">
            <div class="col-lg-10 left-text">
                <div class="row date-text center-align-text" style="font-size:13px;"><?php echo $_SESSION['hotel_data'][$j]['hotel_extra_info']->HotelInfoResult->HotelDetails->HotelName;?></div>
                <div class="row date-text"  ><?php echo $_SESSION['hotel_data'][$j]['hotel_extra_info']->HotelInfoResult->HotelDetails->StarRating;?> stars</div>
            </div>
            ...
            <div class="col-lg-5">
                <div class="row date-text center-align-text" style="font-size:18px;"> <i class="fa fa-inr"></i>&nbsp;<span style="font-family:oswald;font-size:18px;"><?php echo $fare_total[$j];?></span></div>
            </div>
            <hr/>
            <div class="checkTime left-text">
                <span class="h5 heading">Check-In Check-Out Time</span>
                <div class="row check left-text">
                <?php $checkTime = $_SESSION['hotel_data'][$j]['hotel_extra_info']->HotelInfoResult->HotelDetails->HotelPolicy; ?>
                <?php $check = explode('|',$checkTime);
                    $cnt = count($check);
                    for ($i=0;$i<$cnt-1;$i++) {?>
                    <ul>
                                        ...
                                        <div class="col-lg-4 pull-right">
                                            <div class="row fl_btn1">
                                                <div class="link-btn-ed">
                                                    <a href="#" class='btn-ed' tabindex="0" id="edit-btn-<?php echo $n+1;?>">EDIT</a>
                                                </div>
...
(function(){
    ...
    $('#edit-btn-1').on('click', function(){
        var loc = "<?php echo site_url('hotel_api/hotels/isEdit?hotel_num=1');?>";
        window.location.href = loc;
    });
    $('#edit-btn-2').on('click', function(){
        var loc = "<?php echo site_url('hotel_api/hotels/isEdit?hotel_num=2');?>";
        window.location.href = loc;
    });
    $('#edit-btn-3').on('click', function(){
        var loc = "<?php echo site_url('hotel_api/hotels/isEdit?hotel_num=3');?>";
        window.location.href = loc;
    });
    $('#edit-btn-4').on('click', function(){
        var loc = "<?php echo site_url('hotel_api/hotels/isEdit?hotel_num=4');?>";
        window.location.href = loc;
    });
...
//filters section end
    ...
    $.ajax({
        ...
        //three sections are possible x<=8000, 8000<x<=9999999, x>9999999
        ...
        if( maxPrice <= 8000 ){
            rangeTrigger = 0;
            maxPrice = 8000;
        }else if( maxPrice > 8000 && maxPrice <= 9999999 ){
            rangeTrigger = 1;
                            ...
                            +'<div class="custThumbnailContainer" style="background: url('+val.HotelPicture+') center center no-repeat; background-size:cover;">'
                                ...
                                +'<div class="custCaption hotelCaption left-text">'
                                    ...
                                    +'<div class="row">'
                                        ...
                                        +'<div class="col-xs-12 hotelPrice right-text"><i class="fa fa-inr"></i>&nbsp; <span>'+val.Price.PublishedPrice+'</span><div class="onwardsLabel">onwards</div></div>'
    ...
    $( '.results' ).on('click', '.result', function(){
        ...
        $('#room_selected').selectOrDie({
            ...
            onChange: function(){
                ...
                $('.selected-room-info-text').append("<h4>Total Price: <i class='fa fa-inr'></i>&nbsp; "+$('#room_price_disp').html()+" <span class='h5 osw-text'>(<i class='fa fa-inr'></i>&nbsp; "+$('#room_price_per_night_disp').html()+" / Per Night)</span></h4>");
            ...
            $('#book_link').on('shown.bs.tab', function(e) {
                if( e.target.id === 'book_link' ){
                    ...
                    <?php if( isset($_GET['multi_rooms']) ):?>
                        var roomCount = parseInt("<?php echo $_GET['multi_rooms']?>");
                    <?php else:?>
                        var roomCount = 1;
                    <?php endif;?>
                    ...
                    for( var i = 0 ; i < roomCount ; i++ ){
                        ...
                        $("#room_selected-"+i).selectOrDie("destroy");
						...
						$('#room_selected-1').attr('disabled',true);
						$('#room_selected-2').attr('disabled',true);
						$('#room_selected-3').attr('disabled',true);
                        $('#room_selected-'+i).selectOrDie({
                            ...
                            onChange: function(){
                                ...
                                $.ajax({
                                    ...
                                    data: { room_type : room_type, hotel_info : hotel_info, multi_rooms : multi_rooms, trace_id : trace_id, priceDetails: resultdata.GetHotelRoomResult.HotelRoomsDetails[room[0]], index:roomindex}
                                })
                                .done(function( msg ) {
...
//calculates the width of navtabs
    ...
    function room_options(room_selected, dropdownIndex){
        ...
        if( room_selected.GetHotelRoomResult.RoomCombinations.InfoSource === "FixedCombination" ){
            ...
            $.each(room_selected.GetHotelRoomResult.HotelRoomsDetails, function(i, val){
                ret_str += '<option value="'+val.RoomIndex+'~s~'+val.RoomTypeCode+'~s~'+val.RoomTypeName+'~s~'+val.RatePlanCode+'~s~'+val.Price.PublishedPriceRoundedOff+'">'+val.RoomTypeName+'</option>'
            });
        }
        return ret_str;
    }
    ...
    function setRoomPreferences( BlockRoomResult ){
        ...
        $.each(policy, function(i, val){
            policyStr += '<li>'+val.replace('INR', '<i class="fa fa-inr"></i>&nbsp;')+'</li>';
        }); 
        policyStr += "</ul>"
