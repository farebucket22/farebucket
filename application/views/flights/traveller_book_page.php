...
<?php
    ...
    if( is_string($data['fare']) ){
        $fare = json_decode($data['fare'], false);
    }else{
        $jsonFare = json_encode($data['fare']);
        $fare = json_decode($jsonFare, false);
    }

    if( is_string($data['booking_details']) ){
        $booking_details = json_decode($data['booking_details'], false);
    }else{
        $booking_details = $data['booking_details'];
    }

    $firstKeyOfArray = ''.key($booking_details);
    if( $firstKeyOfArray == "rest" ){
        $booking_details = $booking_details->rest->Segment->WSSegment;
    }
    
    $journey_date = date("D, jS M Y", strtotime($data['travel_date']));
    $j_travel_iso_date = date('c', strtotime($data['travel_date']));
	...
	$total_base_fare = $fare->BaseFare;
	$total_tax_fare = $fare->AdditionalTxnFee + $fare->AirTransFee + $fare->Tax + $fare->OtherCharges + $fare->ServiceTax;
	$total_fare = $total_base_fare + $total_tax_fare;
	$total_base_fare = $total_base_fare;
	
    $original_total_fare = $total_fare;
    ...
    $old_conv_charge = ($data['convenience_charge']/100)*$old_total;
    $old_total = $convenience_charge + $old_total;
    $total_fare = $convenience_charge + ($total_fare - 10);

    $_SESSION['flight_db_fare_params']['flight_offered_price'] = $original_total_fare;
    $_SESSION['flight_db_fare_params']['flight_published_price'] = $fare->PublishedPrice;
    $_SESSION['flight_db_fare_params']['flight_convenience_charge'] = $total_fare - $original_total_fare;
    $_SESSION['flight_db_fare_params']["flight_total_Fare"] = $total_fare;	
	$total_base_fare -= 10;
?>
<style>
	.payu-image{
...
<div class="hidden-fields">
    ...
    <?php $max_arr_len = count($booking_details); $iter = 0;?>
    <?php if( is_array($booking_details) ): 
        foreach($booking_details as $ws): ?>
        <?php
            $depTime = date('H:i', strtotime($ws->DepTIme));
            $arrTime = date('H:i', strtotime($ws->ArrTime));
            $depDate = date('d M Y', strtotime($ws->DepTIme));            
            $var1 = strtotime($booking_details[$iter]->DepTIme);
            $var2 = strtotime($booking_details[$iter]->ArrTime);
            $var3 = $var2 - $var1;
            $n = $var3/(60*60);
            $hrs = floor($n);
            $d = 0; 
        ...
        <div class="row center-align-text fl_row">
            ...
            <div class="col-lg-6 left-text">
                ...
                <div class="row date-text"><?php echo $ws->OperatingCarrier;?> - <?php echo $ws->FlightNumber;?></div>
            </div>
        ...
        <?php 
            ...
            echo '<div class="row center-align-text margin-center">';
                ...
                echo '<div class="col-lg-12 remove-padding"> Layover - '.$data['layover'][$iter].' </div>';
                echo '<div class="col-lg-6 remove-padding grey-line"></div>';
            echo '</div>';}
        ?>
        <?php $iter++; endforeach;?>
        <?php else:?>
            <div class="row center-align-text fl_row">
                ...
                <div class="col-lg-6 left-text">
                    <div class="row date-text center-align-text">Flight Number</div>
                    <div class="row date-text center-align-text"><?php echo $booking_details->OperatingCarrier;?> - <?php echo $booking_details->FlightNumber;?></div>
                </div>
                <div class="col-lg-12 travel-text-margin margin-center remove-padding">
                    <div class="row travel-text">
                        <div class="col-lg-11 remove-padding" id="originPop"><?php echo $booking_details->Origin->CityName;?><div class="row center-align-text time_text" id="from"><?php echo $data['from_field']?></div></div>
                        <span class='col-lg-2 gly-ctm glyphicon glyphicon-play'></span>
                        <div class="col-lg-11 remove-padding" id="destinationPop"><?php echo $booking_details->Destination->CityName;?><div class="row center-align-text time_text" id="from"><?php echo $data['to_field']?></div></div>
                    </div>
                </div>
                <div class="col-lg-5">
                    <div class="row dur-text"><?php echo $data['flight_duration_field'];?></div>
                    <div class="row date-text center-align-text"><?php echo date('d M Y', strtotime($data['travel_date']));?></div>
                </div>
            </div>
        <?php endif;?>
    </div>
        ...
        <div id="popoverHiddenTitle" style="display: none">
            <div class="row">
                <div class="col-lg-12 pull-left">
                    <div class="col-lg-4">
                        <?php if( is_array($booking_details) ): ?>
                        <?php $ar_logo = $booking_details[0]->Airline->AirlineCode;?>
                    ...
                    <div class="col-lg-offset-2 col-lg-18 pop-title-text"><?php echo $booking_details[0]->Airline->AirlineName;?></div>
                    <?php else:?>
                    <?php $ar_logo = $booking_details->Airline->AirlineCode;?>
                    <img src="<?php echo base_url('img/AirlineLogo/'.$ar_logo.'.gif');?>" onError="this.src='<?php echo base_url('img/flightIcon.png'); ?>'" alt="jA" width='30px' />
                </div>
                <div class="col-lg-offset-2 col-lg-18 pop-title-text"><?php echo $booking_details->Airline->AirlineName;?></div>
                <?php endif;?>
            </div>
            <div class="col-lg-12 pull-right">
                ...
                <div class="col-lg-12 pop-title-text"><?php echo date('d M Y', strtotime($data['travel_date']));?></div>
            </div>
        </div>
    </div> 
</div>
    ...
    <div class="clear-top">
                            ...
                            <div class="row">
                                <div class="col-lg-offset-4 col-lg-18 center-align-text travel-text">
                                    <div class="row center-align-text">
                                        <div class="col-lg-11 remove-padding srcDest" id="origin"><?php echo $data['origin'];?></div>
                                        <span class='col-lg-2 gly-ctm glyphicon glyphicon-play'></span>
                                        <div class="col-lg-11 remove-padding srcDest" id="destination"><?php echo $data['destination'];?></div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-lg-21 col-lg-offset-3 TravelDate center-align-text"><?php echo $journey_date;?></div>
                            </div>
                            <div class="row">
                                <div class="col-lg-23 col-lg-offset-1 center-align-text TotFare1 fare-padding"><i class="fa fa-inr"></i>&nbsp;<span><?php echo $total_fare;?></span></div>
                            </div>
                        </div>
        ...
        <div class="container">
            <div class="col-xs-24 col-sm-18 col-md-18" id="error_msg">
                <form action="<?php echo site_url('api/flights/book')?>" method="post" id="travellers-form" enctype="multipart/form-data"></form>
            </div>
                        ...
                        <table class="table table-condensed table-custom" >
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="right-text tf-cng discount-value convenience"><i class="fa fa-inr"></i>&nbsp;<span><?php echo $convenience_charge;?></span></td>
                                </tr>
                                <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
                                <tr>
                                    <td>Total Fare <span class="glyphicon glyphicon-question-sign totalNote"></span> </td>
                                    <td class="right-text tf-cng"><div class="row totFare"><strike style="display:none;"><i class="fa fa-inr"></i>&nbsp;<?php echo $total_fare;?></strike></div><div class="row finalFare"><i class="fa fa-inr"></i>&nbsp;<span><?php echo $total_fare;?></span></div></td>
                                </tr>
...
$(document).ready(function(){
    ...
    var temp_date = travel_date.getDate() + 1;
    travel_date.setDate(temp_date);
...
//discount section
    var discountClick = 0;
    var total_fare = parseFloat("<?php echo $total_fare;?>");
    var total_fare_html = "<?php echo $data['total_fare_field'];?>";
        ...
        if( discountCode === "" || discountCode === null ){
            ...
            .done(function(data){
                if(data === "Failure"){
					...
					if( data.message == "Discount cannot be applied for this amount." ){
						$('.discountMessageBlock .errorMessage').html('This discount code cannot be used for this amount.').fadeIn(200);
							setTimeout(function(){
							$('.discountMessageBlock .errorMessage').fadeOut(200);
						}, 3000);
					}
...
//forgot password code
    ...
    $('button.reg-submit').on('click', function(e){
        ...
        if( $('#form-1').data('bootstrapValidator').isValid() ){
			$('button.reg-submit').attr('disabled','disabled');
            var fields = {};
