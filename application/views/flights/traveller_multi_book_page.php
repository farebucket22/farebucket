...
<?php
    if( !isset($_SESSION['flight_data']) ){
		redirect('flights/time_out');
	}
    if( isset($_SESSION['multi_domestic_check']) &&  $_SESSION['multi_domestic_check'] == 0){
        $internationalFlightExists = 1;
    }
    else{
        $internationalFlightExists = 0;
    }
    ...
    for( $i = 0 ; $i < $flights_len ; $i++ ){
        if( isset($_SESSION['flight_data'][$i]['ov']) ){
            if( $_SESSION['flight_data'][$i]['ov']->mode == 'flight' ){
                ...
                $travel_date = $_SESSION['flight_data'][$i]['ov']->travel_date;
            }
        }
        elseif( isset($_SESSION['ovMode']) && $_SESSION['ovMode']->mode == 'flight' ){
            $travel_date = $_SESSION['ovMode']->travel_date;
            $details[$i] = json_decode($_SESSION['ovMode']->booking_details);
            $ov_data[$i] = 1;
        }
    }
        ...
        for( $i = 0 ; $i < $flights_len ; $i++ ){
            if(isset($_SESSION['flight_data'][$i]['ov']) ){
                $flight_ov_data = $_SESSION['flight_data'][$i]['ov'];
            }
            elseif (isset($_SESSION['ovMode']) && $_SESSION['ovMode']->mode == "flight" ) {
                $flight_ov_data = $_SESSION['ovMode'];
            }
            if( $flight_ov_data->mode == 'flight' ){
                if( $same_flag == 0 ){
                    $same_flag = 1;
                }
...
<div class="hidden-fields">
    <?php for( $hc = 0 ; $hc < $flights_len ; $hc++):?>
        <div id="popoverHiddenContent-<?php echo $hc+1;?>" style="display: none">
                <?php if( (isset($_SESSION['flight_data'][$hc]['ov']) && $_SESSION['flight_data'][$hc]['ov']->mode == 'flight'  ) || ( (isset($_SESSION['ovMode'])&& ($_SESSION['ovMode']->mode == "flight") && $ov_data[$hc] == 1 ))) : 
                    if( isset($_SESSION['flight_data'][$hc]['ov']) ) { $flight_ov_data = $_SESSION['flight_data'][$hc]['ov']; }
                    elseif( isset($_SESSION['ovMode']) ) { $flight_ov_data = $_SESSION['ovMode']; }
            ...
            <?php
                ...
                $depDate = date('d M Y', strtotime($flight_ov_data->WSSegment->DepTIme));
                ...
                <div class="row center-align-text fl_row">
                    ...
                    <div class="col-lg-6">
                        ...
                        <?php
                            ...
                            if(isset($flight_ov_data->WSSegment->FlightNumber)){$FlightNumber = $flight_ov_data->WSSegment->FlightNumber;}else{$FlightNumber =  "";}
                        ?>
                        <div class="row date-text"><?php echo $OperatingCarrier;?> - <?php echo $FlightNumber;?></div>
                    </div>
            ...
            <?php elseif( isset($_SESSION['flight_data'][$hc]) && $_SESSION['flight_data'][$hc]['ov']->mode == 'cab'):?>
                <?php
                    $cab_dates = $_SESSION['details']['dates'][$hc];
                ...
                <?php if( $noOfSed ):?>
                    <div class="row center-align-text fl_row">
                        ...
                        <div class="col-lg-6">
                            <div class="row date-text-mod center-align-text">Cab Type</div>
                            <div class="row date-text-mod">Sedan (<?php echo $noOfSed;?>)</div>
                        </div>
                        <div class="col-lg-12 travel-text-margin">
                            <div class="row travel-text">
                                <div class="row date-text-mod center-align-text">Total Pax</div>
                ...
                <?php if( $noOfSuv ):?>
                    <div class="row center-align-text fl_row">
                        ...
                        <div class="col-lg-6">
                            <div class="row date-text-mod center-align-text">Cab Type</div>
                            <div class="row date-text-mod">SUV (<?php echo $noOfSuv;?>)</div>
                        </div>
                        <div class="col-lg-12 travel-text-margin">
                            <div class="row travel-text">
                                <div class="row date-text-mod center-align-text">Total Pax</div>
    ...
    <?php for( $ht = 0 ; $ht < $flights_len ; $ht++):?>
        <?php if( (isset($_SESSION['flight_data'][$ht]) && $_SESSION['flight_data'][$ht]['ov']->mode == 'flight') || ( (isset($_SESSION['ovMode'])) && ($_SESSION['ovMode']->mode =="flight") && ( $ov_data[$ht] === 1 ))) : ?>
            <?php 
                if( isset($_SESSION['flight_data'][$ht]['ov']) && $_SESSION['flight_data'][$ht]['ov']->mode == 'flight' ) { $flight_ov_data = $_SESSION['flight_data'][$ht]['ov']; }
                elseif( isset($_SESSION['ovMode']) ) { $flight_ov_data = $_SESSION['ovMode']; }
            ?>
    ...
    <div class="clear-top">
                    ...
                    <?php for( $n = 0 ; $n < $flights_len ; $n++):?>
                        <div class="col-lg-5 fl_septr1">
                            ...
                            <div class="col-lg-14 fl_info">
                                <div class="row">
                                    <div class="col-lg-offset-4 col-lg-18 travel-text">
                                        <div class="row center-align-text">
                                            <div class="col-lg-11 remove-padding ellipse" id="origin<?php echo $n+1;?>"><?php if(isset($_SESSION['flight_data'][$n]['ov'])) {echo $_SESSION['flight_data'][$n]['ov']->org;} elseif (isset($_SESSION['ovMode']) && $_SESSION['ovMode']->mode=="flight" ) {echo $_SESSION['ovMode']->org;}?></div>
                                            <span class='col-lg-2 gly-ctm glyphicon glyphicon-play'></span>
                                            <div class="col-lg-11 remove-padding ellipse" id="destination<?php echo $n+1;?>"><?php if(isset($_SESSION['flight_data'][$n]['ov'])) {echo $_SESSION['flight_data'][$n]['ov']->dest;} elseif (isset($_SESSION['ovMode'])) {echo $_SESSION['ovMode']->dest;}?></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                ...
                                <div class="row">
                                    <?php if( isset($_SESSION['flight_data'][$n]['ov']->mode) && $_SESSION['flight_data'][$n]['ov']->mode == 'bus' ):?>
                                        <div class="col-lg-23 col-lg-offset-1 center-align-text fare-padding TotFare1"><i class="fa fa-inr"></i>&nbsp;<span><?php echo $_SESSION['flight_data'][$n]['ov']->fareDet['totalFare'];?></span></div>
                                    <?php elseif(isset($_SESSION['flight_data'][$n]['ov']->mode) && $_SESSION['flight_data'][$n]['ov']->mode == 'cab'):?>
                                        <div class="col-lg-23 col-lg-offset-1 center-align-text fare-padding TotFare1"><i class="fa fa-inr"></i>&nbsp;<span><?php echo $_SESSION['flight_data'][$n]['ov']->totalFare;?></span></div>
                                    <?php else:?>
                                        <div class="col-lg-23 col-lg-offset-1 center-align-text fare-padding TotFare1"><i class="fa fa-inr"></i>&nbsp;<span><?php if(isset($_SESSION['flight_data'][$n])) {echo $_SESSION['flight_data'][$n]['total_fare_field'];} elseif ( isset($_SESSION['ovMode']) ) {echo $_SESSION['ovMode']->total_fare_field;} ?></span></div>
                                    <?php endif;?>
                                </div>
                            </div>
                            <div class="col-lg-4 pull-right">
                                <div class="row fl_btn1">
                                    <div class="link-btn-de link-btn-de-traveller">
                                        <a class='btn-de' tabindex="0" id='popover-toggle-<?php echo $n+1;?>' data-toggle="popover" data-placement="bottom">DETAILS</a>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="container">
            <div class="col-xs-24 col-sm-14 col-md-18">
                ...
                <?php else:?>
                    <form action="<?php echo site_url('common/payment_gateway')?>" method="post" id="travellers-form" enctype="multipart/form-data">
                        <input name="details" value="" style="display:none;"/>
                        <input name="key" value="LcXB2s" style="display:none;"/>
                        <input name="multiplex_total_fare" value="<?php echo $total_fare_field;?>" style="display:none;"/>
                        <input name='productinfo' type='text' value='multiplexed tickets' style="display:none;"/>
                        ...
                        <table class="table table-condensed table-custom" >
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="right-text tf-cng discount-value"><i class="fa fa-inr"></i>&nbsp;<?php echo $convenience_charge;?></td>
                                </tr>
                                <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
                                <tr>
                                    <td>Total Fare <span class="glyphicon glyphicon-question-sign totalNote"></span> </td>
                                    <td class="right-text tf-cng"><div class="row totFare"><strike style="display:none;"><i class="fa fa-inr"></i>&nbsp;<?php echo $total_fare_field;?></strike></div><div class="row finalFare"><i class="fa fa-inr"></i>&nbsp;<span><?php echo $total_fare_field;?></span></div></td>
                                </tr>
...
$(document).ready(function(){
    ...
    //internation flights
    var international_var = <?php echo $internationalFlightExists;?>;

    if( international_var == 0 ){
    ...
    <?php else:?>
        ...
        {
            var j = i + 1;
            $('#travellers-form').append(' <div class="form-padding row"> <div class="col-lg-24"> <h4> Passenger '+j+' </h4> </div></div><div class="form-padding row"> <div class="col-lg-3 col-md-3 col-sm-6 col-xs-16 form-padding title_div"> <select class="selectpicker form-control title traveller_title_assorted" id="not_selected" name="Title'+i+'"> <option>Title</option> <option> Mr </option> <option> Ms </option> <option> Mrs </option> </select> </div><div class="col-lg-8 col-md-8 col-sm-9 col-xs-23 form-padding"> <input type="text" class="form-control" name="fname'+i+'" placeholder="First Name"> </div><div class="col-lg-8 col-md-8 col-sm-9 col-xs-23 form-padding"> <input type="text" class="form-control" name="lname'+i+'" placeholder="Last Name"> </div></div><div class="form-padding row"> <div class="col-lg-3 col-md-3 col-sm-9 col-xs-23 form-padding"> <select class="selectpicker form-control gender" id="not_selected" name="sex'+i+'"> <option>Gender</option> <option> Male </option> <option> Female </option> </select> </div><div class="col-lg-4 col-md-4 col-sm-9 col-xs-23 form-padding"> <input type="text" class="form-control" name="dob'+i+'" id="age_dob'+i+'" placeholder="DOB"> <input type="text" class="form-control" name="age'+i+'" id="age_dob_hid'+i+'" style="display:none;"></div></div><div class="form-padding row passportContent"> <div class="col-lg-10 col-md-10 col-sm-9 col-xs-23 control-label form-padding"> <div class="form-group"> <input type="text" class="form-control" name="pass_number'+i+'" placeholder="Passport Number"> </div></div><div class="col-lg-10 col-md-10 col-sm-9 col-xs-23 control-label form-padding"> <div class="inner-addon right-addon"><i class="glyphicon"></i> <div class="form-group"> <input name="pass_expiry'+i+'" readonly id="expiry-date-'+i+'" class="form-control" type="text" placeholder="Passport Expiry Date"> </div></div></div><div class="col-lg-4 col-md-4 col-sm-6 col-xs-16 form-padding"></div>');
        };

        $('#travellers-form').append('<div class="col-lg-10 col-md-10 col-sm-9 col-xs-23 form-padding"> </div>');
        $('#travellers-form').append('<div class="form-padding row"> <div class="col-lg-24"> <h4> Contact Details </h4> </div></div><div class="form-padding row"> <div class="col-lg-8 col-md-12 col-sm-12 col-xs-23 form-padding"> <input type="text" class="form-control forceNumeric" name="mobile" placeholder="Mobile Number"> </div><div class="col-lg-8 col-md-12 col-sm-12 col-xs-23 form-padding"> <input type="text" class="form-control" name="email_id" placeholder="Email Id"> </div></div><div class="form-padding row"> <div class="col-lg-16 col-md-16 col-sm-24 col-xs-23 form-padding"> <textarea rows="4" cols="25" type="text" class="form-control form-padding" name="addressPickup" placeholder="Pickup Address"></textarea> <textarea rows="4" cols="25" type="text" class="form-control form-padding" name="addressDrop" placeholder="Drop Address"></textarea> </div></div><div class="form-padding row"> <div class="col-lg-4 col-md-12 col-sm-12 col-xs-23 form-padding"> <select name="id_proof" id="proof not_selected" class="form-control idproof" type="text" placeholder="ID Proof"> <option> ID Proof </option> <option> Pan Card </option> <option> Driving Licence </option> <option> Voting Card </option> <option> Adhar Card </option> </select> </div><div class="col-lg-4 col-md-12 col-sm-12 col-xs-23 form-padding"> <input type="text" class="form-control" name="id_no" placeholder="Id Number"> </div></div><div class="row"> <div class="form-group form-padding col-xs-2 col-sm-1 col-md-2 col-lg-12"> <label class="pickupControl">Pickup Date / Time</label> </div></div><div class="row pickRow"> <div class="col-lg-4"> <div class="inner-addon right-addon"> <i class="glyphicon"></i> <input name="pickupDate" id="date-1" readonly class="form-control form-padding pickupdate" type="text" placeholder="Depart Date" value="<?php echo $cab_dates;?>"> </div></div><div class="form-group form-padding col-xs-2 col-sm-1 col-md-2 col-lg-3"> <select name="timeHours" id="not_selected" class="timeHours form-padding small-input-prefix forceNumeric hoursMins"> <option>Hours</option> <?php for($i=0;$i<24;$i++):?> <option value="<?php echo $i; ?>"><?php $j=sprintf("%02d",$i); echo $j ?></option> <?php endfor;?> </select> </div><div class="form-group form-padding col-xs-2 col-sm-1 col-md-2 col-lg-3"> <select name="timeMins" id="not_selected" class="timeMins form-padding small-input-prefix forceNumeric hoursMins"> <option>Minutes</option> <?php for($i=5;$i<56;$i+=5):?> <option><?php $j=sprintf("%02d",$i); echo $j ?></option> <?php endfor;?> </select> </div></div>');
        ...
        <?php for ( $i = $adult ; $i < ($adult+$child) ; $i++): ?>
            $('#age_dob<?php echo $i;?>').datepicker({
                changeYear: true,
                changeMonth: true,
                maxDate: '-2Y',
        ...
        <?php for ( $i = ($adult+$child) ; $i < ($adult+$child+$infant); $i++): ?>
            $('#age_dob<?php echo $i;?>').datepicker({
                changeYear: true,
                changeMonth: true,
                maxDate: 0,
...
$('#form-1').bootstrapValidator({
    ...
    $('#travellers-form').on('click', '#form-submit-button', function(e){
        ...
        var addr = $('textarea[name=addressPickup]').val();
        if(addr === undefined){
            console.log(addr);
        }
        else{
            addr = $('textarea[name=addressPickup]').val().replace(/\n/g, " ");
            $('textarea[name=addressPickup]').val('');
            $('textarea[name=addressPickup]').val(addr);
        }
        ...
        if( not_selected_flag ){
            $.each( all_selects, function(i, val){
                if( val.id === 'not_selected' || val.id === 'proof not_selected' ){
                    $(val).parent().siblings('small.help-block').html('This field is required');
    ...
    var travel_date = "<?php if( isset($travel_date) ) echo $travel_date; else { ""; } ?>";

    <?php if( isset($_SESSION['details']['total_count']) ):?>
        <?php for ( $i = 0 ; $i < $_SESSION['details']['total_count'] ; $i++): ?>
            $('#expiry-date-<?php echo $i;?>').datepicker({
                minDate: travel_date,
                changeMonth: true,
                changeYear: true,
                yearRange: 'c:c+100',
    ...
    var discountClick = 0;
    var total_fare = parseFloat("<?php echo $total_fare_field;?>");
...
// this part checks the placeholder of a field and add various validation options eg: is e-mail validation for e-mail etc
    ...
    <?php if( !$cab_present ):?>
        ...
        $('label[class=pickupControl]').hide();
        $('.pickRow').hide();
    <?php endif;?>
});
</script>