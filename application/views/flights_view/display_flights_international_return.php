<style>
 ...
 <form action="<?php echo site_url('api/flights/return_international_traveller_details');?>" id="booking_details_form" method="post" style="display:none;">
                                        ...
                                        <div class="col-lg-14 fl_info">
                                            <div class="row">
                                                <div class="col-lg-offset-4 col-lg-18 center-align-text travel-text">
                                                    <div class="row center-align-text">
                                                        <div class="col-lg-6 remove-padding srcDest" id="origin1"></div>
                                                        <span class='col-lg-3 gly-ctm glyphicon glyphicon-play'></span>
                                                        <div class="col-lg-6 remove-padding srcDest" id="destination1"></div>
                                                        <span class='col-lg-3 gly-ctm glyphicon glyphicon-play'></span>
                                                        <div class="col-lg-6 remove-padding srcDest" id="destination2"></div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="row">
                        ...
                        <div class="row">
                            <div class="panel-group" id="accordion">
                                ...
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h4 class="panel-title">
                                            <a class="accordion-toggle" data-toggle="collapse" data-target="#collapseArr">
                                                Arrival Time
                                            </a>
                                        </h4>
                                    </div>
                                    <div id="collapseArr" class="panel-collapse filterCollapse collapse in">
                                        <div class="panel-body">
                                            <div class="simple">
                                                <div class="range-slide-arr" id="slider-range-duration-arr"></div>
                                                <span id="range-bottom-duration-arr" class="range-value-arr"></span> to <span id="range-top-duration-arr" class="range-value"></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
// remove last sorting icon end
    ...
    $('#search tbody').on('click', '.book_btn', function(){
        ...
        $('#booking_details_form input[name=outbound_from_field]').val($('#row-'+index+' td #out_from').html());
        $('#booking_details_form input[name=outbound_to_field]').val($('#row-'+index+' td #out_to').html());
        $('#booking_details_form input[name=outbound_flight_duration_field]').val($('#row-'+index+' td #out_dur').html());
        ...
        $('#booking_details_form input[name=inbound_from_field]').val($('#row-'+index+' td #in_from').html());
        $('#booking_details_form input[name=inbound_to_field]').val($('#row-'+index+' td #in_to').html());
        $('#booking_details_form input[name=inbound_flight_duration_field]').val($('#row-'+index+' td #in_dur').html());
    ...
    var from_full_str = from_raw.slice(0, comma1);
    var to_full_str = to_raw.slice(0, comma2);
	var utf_to = "<?php echo $_GET['utf_to'];?>";
    var utf_from = "<?php echo $_GET['utf_from'];?>";
    ...
    var TravelDateOutbound = "outbound: " + "<?php echo date('jS M Y', strtotime($_GET['to_date']));?>";
    $('.TravelDateOutbound').html(TravelDateOutbound);
    var TravelDateInbound = "inbound: " + "<?php echo date('jS M Y', strtotime($_GET['from_date']));?>";
    ...
    if(res !== 'false')
    {
        ...
        max_durationArr = parseInt(result.max_durationArr) + 10;
        min_durationArr = parseInt(result.min_durationArr) - 10;
        ...
        $.each(result.travel_connecting, function(i,place){
            origin[i] = place.origin,
            dest[i] = place.destination
        });

        $.each(result.results, function(i, value){
			var first = value.multi[0].airline;
            $.each(value.multi, function(j, time){
				if(time.airline !== first){
                    value.multiple_carriers = 1;
                }
                if(time.destination === search_parameters.to || time.destination_city === search_parameters.to){
                    to_dest[i] = time.to;
                }
                if(time.origin === search_parameters.to || time.origin_city === search_parameters.to){
                    from_src[i] = time.from;
                }
            });
        });
        ...
        $.each(result.results, function( i, val ){
            oTable.row.add([
                val.airline,
                '<div class="row logo"><img width="32px" src="<?php echo base_url("img/AirlineLogo/'+val.airline_code+'.gif");?>" onError='+flight_img+'></div><div class="row air">'+val.airline+'</div>',
                '<div class="row">'+origin[i]+'-'+dest[i]+'</div><div class="row">'+dest[i]+'-'+origin[i]+'</div>',
                 val.from_hidden,
                '<div class="row" id="out_from">'+val.from+'</div><div class="row" id="in_from">'+from_src[i]+'</div>',
                val.to_hidden,
                '<div class="row" id="out_to">'+to_dest[i]+'</div><div class="row" id="in_to">'+val.to+'</div>',
                '<div class="row" id="out_dur">'+val.duration_out+'</div><div class="row" id="in_dur">'+val.duration_in+'</div>',
                val.fare,
                '<div class="row flight_fare"><i class="fa fa-inr"></i>&nbsp;<span id="fare-'+i+'" class="currency">'+parseFloat(val.fare).toFixed(2)+'</span></div><div class="row"><h6>Details</h6></div>',
                val.stops,
                val.ticket_type,
                '<button id="button-'+i+'" class="btn btn-change book_btn" type="button">BOOK</button>'
            ]).draw();
        });
		
		$.each(result.results, function(i, val){
            $('#search #row-'+i+' td .flight_fare #fare-'+i+'.currency').html(price);
            if(val.multiple_carriers === 1){
                $('#search #row-'+i+' td .logo img').attr("src","<?php echo base_url('img/flightIcon.png');?>");
                $('#search #row-'+i+' td .air').html("Multiple Carriers");
            }
        });
...
//tooltip code end
    ...
    //weather end
        ...
        "bDeferRender": true,
        "aoColumnDefs": [
            { "aDataSort": [ 8, 9 ], "aTargets": [ 9 ] },
            { "targets": [ 0, 3, 5, 8, 10, 11 ], "visible": false },
    ...
    var max_duration = 0;
    var min_duration = 0;
    var max_durationArr = 0;
    var min_durationArr = 0;
    ...
    var rangeSliderDurationArr = $( "#slider-range-duration-arr" );

    function resetSlider() {
        var $slider = $("#slider-range");
        var $sliderDuration = $("#slider-range-duration");
        var $sliderDurationArr = $("#slider-range-duration-arr");
        ...
        $sliderDurationArr.slider("values", 0, min_durationArr);
        $sliderDurationArr.slider("values", 1, max_durationArr);
    }
    ...
    function format (d, row_details_id) {
        ...
        if( from_full_str == "Madras" ){
            from_full_str = "Chennai";
        }
        if( to_full_str == "Madras" ){
            to_full_str = "Chennai";
        }
                            ...
                            +'<div class="col-lg-16">'
                                +'<div class="row">'
                                    +'<div class="col-lg-6 std-lineheight left-text">'
                                        +'<span class="inline-h4">'+from_full_str+'</span> <span class="glyphicon glyphicon-arrow-right"></span> <span class="inline-h4">'+to_full_str+'</span>'
                                    +'</div>'
                                ...
                                +'<div class="row">'
                                    ...
                                    +'<div class="col-lg-4 center-align-text"></div>'
                ...
                for (var i=1  ; i <= return_data.multi_flight_info[ind].length ; i++) {
                    if( return_data.layover[ind].length > 1 ){
                        if( i === return_data.multi_flight_info[ind].length ){
                            return_row_element[i] = 
                            '<div class="row">'
                                +'<div class="col-lg-6 left-text"><div>'+return_data.multi_flight_info[ind][i-1].name_of_airline+'</div><div class="text-small">'+return_data.details[ind].rest.Segment.WSSegment[i-1].OperatingCarrier+'-'+return_data.details[ind].rest.Segment.WSSegment[i-1].FlightNumber+'</div></div>'
                            ...
                            if( parseInt(return_data.details[ind].rest.ObSegCount) == i ){
                                return_row_element[i] = 
                                '<div class="row">'
                                    +'<div class="col-lg-6 left-text"><div>'+return_data.multi_flight_info[ind][i-1].name_of_airline+'</div><div class="text-small">'+return_data.details[ind].rest.Segment.WSSegment[i-1].OperatingCarrier+'-'+return_data.details[ind].rest.Segment.WSSegment[i-1].FlightNumber+'</div></div>'
                                    +'<div class="col-lg-7 right-text"><div class="row">'+return_data.results[ind].multi[i-1].from+' <span class="ctCode">'+return_data.travel_connecting[ind].connecting[i-1].origin+'</span></div><div class="row sd-details-line-height">'+return_data.multi_flight_info[ind][i-1].source_details+'</div><div class="row">'+return_data.multi_flight_info[ind][i-1].dep_date+'</div></div>'
                                    +'<div class="col-lg-4 center-align-text"></div>'
                                    +'<div class="col-lg-7 left-text"><div class="row"><span class="ctCode">'+return_data.travel_connecting[ind].connecting[i-1].destination+'</span> '+return_data.results[ind].multi[i-1].to+'</div><div class="row sd-details-line-height">'+return_data.multi_flight_info[ind][i-1].destination_details+'</div><div class="row">'+return_data.multi_flight_info[ind][i-1].arr_date+'</div></div>'
                                    +'<div class="col-lg-24"><div class="sr_sptr"></div></div>'
                                    +'<div class="col-lg-6 std-lineheight userRegisterContainer left-text">'
                                        +'<span class="inline-h4">'+to_full_str+'</span> <span class="glyphicon glyphicon-arrow-right"></span> <span class="inline-h4">'+from_full_str+'</span>'
                                    +'</div>'
                                +'</div>'
                            }else{
                                return_row_element[i] = 
    ...
    $.fn.dataTableExt.afnFiltering.push(
        function( oSettings, aData, iDataIndex ) {
            var iColumn = 5;
            var iMin = rangeSliderDurationArr.slider("values", 0);
            var iMax = rangeSliderDurationArr.slider("values", 1);
    ...
    var initRangeSliderDurationArr =  function(){
        rangeSliderDurationArr.slider({
           range: true,
           min: min_durationArr,
           max: max_durationArr,
           step: 10,
           values: [ min_durationArr, max_durationArr ],
           slide: function( event, ui ) {
                //to insert a colon inbetween the text
                var rangeBottomDurationStrArr = ui.values[ 0 ].convertToTime();
                var rangeTopDurationStrArr = ui.values[ 1 ].convertToTime();

                $('#range-bottom-duration-arr').html(rangeBottomDurationStrArr);
                $('#range-top-duration-arr').html(rangeTopDurationStrArr);
           },
           stop: function(event, ui) {
                flashTable();
                oTable.draw();
    ...
    var rangeBottomDurationArr = rangeSliderDurationArr.slider("values", 0);
    var rangeTopDurationArr = rangeSliderDurationArr.slider("values", 1);

    $('#range-bottom-duration-arr').html(min_durationArr.convertToTime());
    $('#range-top-duration-arr').html(max_durationArr.convertToTime());
