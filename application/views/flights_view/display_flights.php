<?php 
    if(!isset($_SESSION['oneway_utf_from']) || $_GET['utf_from'] !== 0 ){
        $_SESSION['oneway_utf_from'] = $_GET['utf_from'];
    }
    if(!isset($_SESSION['oneway_utf_to']) || $_GET['utf_to'] !== 0 ){
        $_SESSION['oneway_utf_to'] = $_GET['utf_to'];
    }
 ?>
<style type="text/css">
    .modal-content{
...
<form action="<?php echo site_url('api/flights/traveller_details');?>" id="booking_details_form" method="post" style="display:none;">
    ...
    <input class="form-control" name="total_count_field" type="hidden" value="<?php echo ($_GET['adult_count'] + $_GET['youth_count'] + $_GET['kids_count']);?>"/>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body hotelInfoModalBody">
                <div class="container-fluid fareRuleModalBody">
                    <div class="row flight_details"></div>
                </div>
            </div>
        </div>
    </div>
...
======================================================================*/
    ...
    var total_count = "<?php echo $_GET['adult_count'] + $_GET['youth_count'] + $_GET['kids_count'];?>";
    ...
    .done(function (flightData) {
        ...
        isInternational = result.isDomestic;
        ...
        if( result !== false ){
            ...
            $.each(result.general, function(key, val){
                ...
                oTable.row.add([
                    ...
                    val.Duration,
                    val.TotalFare,
                    '<div class="row flight_fare"><i class="fa fa-inr"></i>&nbsp;<span id="fare-'+key+'" class="currency">'+parseFloat(val.TotalFare).toFixed(2)+'</span></div><div class="row"><h6>Details</h6></div>',
                    val.StopsCount,
                    val.NonRefundable,
                    bookBtn
                ]).draw();
...
==========================================================*/
    ...
    $(document).on('resetAllFilters', function(){
        var airlinesContainer = $('#airlines-check');
        var airlineStopsContainer = $('#stops-check');
        var airlineTicketTypeContainer = $('#ticket-check');
        var airlinesCheckboxes = airlinesContainer.find(':input');
        var airlineStopsCheckboxes = airlineStopsContainer.find(':input');
        var airlineTicketTypeCheckboxes = airlineTicketTypeContainer.find(':input');

        $.each(airlinesCheckboxes, function(i, val){
            if( val.checked === false ){
                val.click();
            }
        });
        ...
        $.each(airlineTicketTypeCheckboxes, function(i, val){
            if( val.checked === false ){
                val.click();
            }
        });
...
=======================================================*/
    ...
    $('#search tbody').on('click', 'td', function () {
            ...
            if ( row.child.isShown() ) {
                ...
                // Open this row
                row.child( format(flightSegment[selectedRowId], fare[selectedRowId], generalInfo[selectedRowId], layover[selectedRowId], segmentInfo[selectedRowId], selectedRowId, form_full_str, to_full_str, isInternational) ).show();
                tr.addClass('shown');
                tr.next('tr').addClass('tr-drawer');
                $('.fareRule_btn').on('click', function(){
                    $('.tr-drawer').removeClass('shown');
                    $('#flight_details_modal').modal('show');
                    $('.hotels_spinner').show();
                        $('#flight_details_modal .modal-header').addClass('hide');                        
                        $('#flight_details_modal .modal-content').addClass('hide');

                    $.ajax({
                        type:'post',
                        url:'<?php echo site_url("flights_api/api/getFareRule");?>',
                        data: {rowid:selectedRowId, wsres: wsresult[selectedRowId]},
                    })
                    .done(function(result){
                        var res = $.parseJSON(result);
                        $('.hotels_spinner').hide();
                        $('#flight_details_modal .modal-header').removeClass('hide');                        
        ...
        geocoder.geocode( { 'address': destination_city_name }, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                ...
                .done(function(weatherdata){
                    ...
                    if( wData.isHistoric ){
                        // historic weather
                        $('.weatherDetails .from .date').html('<div class="title">Weather for:</div><span>'+destination_city_name+'<br />(Today)</span>');
                        ...
                        $('.weatherDetails .to .date').html('<div class="title">Weather for:</div><span>'+source_city_name+'<br />(Today)</span>');
