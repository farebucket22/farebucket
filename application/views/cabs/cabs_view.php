<head>
    <title>Farebucket:Cheap Cabs In Bangalore | Cheap Domestic Air Tickets</title>
    <meta name="Subject" content="Cheap Cabs In Bangalore" />
    <meta name="description" content="Farebucket has concentrated its efforts in producing quality travel, responding to the needs of the times while perceiving the demands of the future. Cheap Cabs In Bangalore | Cheap Domestic Air Tickets" />
                ...
                <form action="<?php echo site_url('cab_api/cabs/search_cabs');?>" method="get" id="form-submit-2" enctype="multipart/form-data">
                    <div class="form-padding row">
                        <div class="form-group">
                            <div class="col-lg-8 control-label has-error outstation_source">
    ...
    .done(function(retDat){
        ...
        sel_opt.selectOrDie({
            ...
            onChange: function(){
                ...
                $('.outstation_source small.help-block').hide();
                ...
                if( outStationSourceClick === 0 ){
                    ...
                    .done(function(retDat){
                        retDat = $.parseJSON(retDat);
                        var sel_opt = $('#outstat_cab_dest');
                        sel_opt.html('<option value="-1" id="sel_def_dest_outStat" selected>Select Destination</option>');
                        $.each(retDat.statename, function(i, val){
                            sel_opt.append("<option id='"+retDat.stateid[i]+"' value='"+retDat.stateid[i]+"-"+retDat.statename[i]+"'>"+val+"</option>");
                        });
                        sel_opt.selectOrDie('destroy');
...
//destination ajax call verification
    $('.form-submit-button-outstation').on('click', function(){
        ...
        if( outStationSourceClick > 0 && outStationDestinationClick > 0 ){
            ...
            if( outStationSourceClick == 0 ){
                $('.outstation_source small.help-block').show();
            ...
            if( $('#date-2').val() == "" ){
                $('.outstation_date small.help-block').show();
                $('#date-2').parent('.sod_select').css('border-color', '#f00');
