...
// setting the user email id to a variable for global use
if( isset($_SESSION['user_details']) && isset($_SESSION['user_details'][0]) ){
    $loggedUserEmail = $_SESSION['user_details'][0]->user_email;
}
?>
...
<style>
    ...
    #flightsTable th, #busesTable th, #cabsTable th, #hotelsTable th, #activitiesTable th{
        width: 0px;
        text-align: center;
        line-height: 1.42em;
        font-size: 12px;
...
<div class="modal fade" id="cancel_modal">
    <div class="modal-dialog">
        <div class="modal-content modal-content-cancellation">
            <div class="modal-header">
                <span class="h4">Confirm Ticket Cancellation</span>
            ...
            <div class="modal-body">
                <div class="textSection">
                    <div class="row"><center class="displayText"></center></div>
                    <div class="row cancelBtnMargin"><div class="center-align-text"><div class="col-lg-11 right-text"><button type='button' class="confirmCancel btn btn-change" id="" data-module="0" >Confirm</button></div><div class="col-lg-offset-2 col-lg-8 left-text"><button type='button' data-dismiss="modal" class="btn btn-change">Cancel</button></div></div></div>
                </div>
                <div class="loadingIcon">
                    <div id="loading_spin" class="spinner spinnerWithinModal" style="display:none;">
                        <div class="rect1"></div>
                        <div class="rect2"></div>
                        <div class="rect3"></div>
                        <div class="rect4"></div>
        ...
        if(flight_click == 1){
            ...
            .done(function( data ) {
                ...
                $.each(retData, function(i, val){
                    ...
                    if( val.fb_bookingId !== null || val.fb_bookingId !== "" ){
                        <?php $ticketUrl = site_url('api/flights/guest_details?guest_email=').$loggedUserEmail;?>
                        var ticketLink = '<a title="Generate Ticket" href="<?php echo $ticketUrl;?>' +'&ticket_id='+ val.fb_bookingId + '">'+val.fb_bookingId+'</a>';
                    }
...
// populating buses table
    ...
    $('#busesTab').on('shown.bs.tab', function(e) {
        ...
        ++bus_click;
        var userId = "<?php echo $userId;?>";
        $('#cancel_modal .confirmCancel').attr('data-module', '1');
        if(bus_click == 1){
            $.ajax({
                type: "POST",
            ...
            .done(function( data ) {
                ...
                $.each(retBusData, function(i, val){
                    ...
                    if( val.fb_bookingId !== null || val.fb_bookingId !== "" ){
                        <?php $ticketUrl = site_url('bus_api/buses/ticket_page?guest_email=').$loggedUserEmail;?>
                        var ticketLink = '<a title="Generate Ticket" href="<?php echo $ticketUrl;?>' +'&ticket_id='+ val.fb_bookingId + '">'+val.fb_bookingId+'</a>';
                    }
...
// ticket cancellation for buses
    ...
    var busOriginalBtn;
    var busCancellationPolicy;

    $('tbody').on('click', '.cancel_bus_ticket', function(e){
        var btn_id = $(this).data('busbookingnum');
        busOriginalBtn = $(this);
        busCancellationPolicy = $(this).data('cancellationpolicy');
        var cancellation_charge = busCancellation(btn_id);
        var refundAmt = parseInt(retBusData[btn_id].totalFare) - cancellation_charge;
        $('#cancel_modal center.displayText').html('Are you sure you want to cancel this ticket? An amount of Rs.'+cancellation_charge+' will be deducted as cancellation charge.');
        $('#cancel_modal .confirmCancel').attr('id', btn_id);
        $('#cancel_modal .textSection .cancelBtnMargin').show();
        $('#cancel_modal').modal('show');
    });

    //ok was clicked
    $('#cancel_modal .confirmCancel').on('click', function(){
        if( $(this).data('module') === 1 ){
            ...
            $('#cancel_modal .textSection').hide();
            $('#cancel_modal #loading_spin').show();
            $.ajax({
                type:"POST",
            ...
            .done(function( retVal ){
                ...
                if( retChk === "Ticket has been cancelled successfully." ){
                    $('#cancel_modal center.displayText').html('Your booking has been cancelled successfully');
                    $('#cancel_modal .textSection').show();
                    $('#cancel_modal #loading_spin').hide();
                    busOriginalBtn.removeClass('cancel_ticket');
                    busOriginalBtn.attr('disabled', 'disabled');
                    busOriginalBtn.html('Cancelled');
                }else{
                    if( retChk.trim() === 'Ticket is already cancelled.' ){
                        $('#cancel_modal center.displayText').html(retChk);
                        $('#cancel_modal .textSection').show();
                        $('#cancel_modal .textSection .cancelBtnMargin').hide();
                        $('#cancel_modal #loading_spin').hide();
                        busOriginalBtn.attr('disabled', 'disabled');
                        busOriginalBtn.html('Cancelled');
                    }
...
// populating cabs table
    ...
    $('#cabsTab').on('shown.bs.tab', function(e) {
        ...
        ++cab_click;
        var userId = "<?php echo $userId;?>";
        $('#cancel_modal .confirmCancel').attr('data-module', '2');
        if(cab_click == 1){
            $.ajax({
                type: "POST",
            ...
            .done(function( data ) {
                ...
                $.each(retCabData.cabBookingDetails, function(i, val){
                    ...
                    if( val.fb_bookingId !== null || val.fb_bookingId !== "" ){
                        <?php $ticketUrl = site_url('cab_api/cabs/getCabTicket?guest_email=').$loggedUserEmail;?>
                        var ticketLink = '<a title="Generate Ticket" href="<?php echo $ticketUrl;?>' +'&ticket_id='+ val.fb_bookingId + '">'+val.fb_bookingId+'</a>';
                    }
        ...
        if( $(this).data('module') === 2 ){
            ...
            .done(function( retVal ){
                ...
                if( retChk === "successful" ){
                    ...
                    $('#cancel_modal .textSection').show();
                    $('#cancel_modal #loading_spin').hide();
                    cabOriginalBtn.removeClass('cancel_ticket');
                    cabOriginalBtn.attr('disabled', 'disabled');
                    cabOriginalBtn.html('Cancelled');
                    return true;
                }else if( retChk === "failed" ){
                    $('#cancel_modal center.displayText').html('Error, please try later.');
                    $('#cancel_modal .textSection').show();
                    $('#cancel_modal .textSection .cancelBtnMargin').hide();
                    $('#cancel_modal #loading_spin').hide();
                    return false;
        ...
        if(hotel_click == 1){
            ...
            .done(function( data ) {
                ...
                $.each(retHotelData.hotelBookingDetails, function(i, val){
                    ...
                    if( val.fb_bookingId !== null || val.fb_bookingId !== "" ){
                        <?php $ticketUrl = site_url('new_request/ticket_page?guest_email=').$loggedUserEmail;?>
                        var ticketLink = '<a title="Generate Ticket" href="<?php echo $ticketUrl;?>' +'&ticket_id='+ val.fb_bookingId + '">'+val.fb_bookingId+'</a>';
                    }
...
// ticket cancellation for hotel
    $('tbody').on('click', '.cancel_hotel_ticket', function(e){
        ...
        if( confirm('Are you sure you want to cancel this ticket?') ){
            ...
            .done(function( retVal ){
                ret = $.parseJSON(retVal);
                if( ret ){
                    alert('Your booking has been cancelled successfully.');
                    originalBtn.removeClass('cancel_hotel_ticket');
...
//search functionality end
    ...
    function cabCancellation(index){
        ...
        if( diffHours >= 4 ){
            ...
            cancellation_charge = cabData.offered_price;
        }
        return cancellation_charge;
    }   
</script>