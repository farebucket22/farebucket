<?php
    class GetReturnTicketDetails{
        ...
        public function setReturnTicketDetails($data,$details,$in_out){
            ...
            if (!is_array($data->rest->FareRule->WSFareRule)) {
                ...
                for ($i = 0; $i < count($data->rest->FareRule->WSFareRule); $i++) {
                    ...
                    // $ticket['Ticket']['wsTicketRequest']["FareRule"]["WSFareRule"]["Destinaton"] = $data['details']->booking_details->rest->FareRule->WSFareRule->Destinaton;

                    $ticket['Ticket']['wsTicketRequest']["FareRule"]["WSFareRule"][$i]["Airline"] = $data->rest->FareRule->WSFareRule[$i]->Airline;

                    // $ticket['Ticket']['wsTicketRequest']["FareRule"]["WSFareRule"]["FareRestriction"] = $data['details']->booking_details->rest->FareRule->WSFareRule->FareRestriction;

                    $ticket['Ticket']['wsTicketRequest']["FareRule"]["WSFareRule"][$i]["FareBasisCode"] = $data->rest->FareRule->WSFareRule[$i]->FareBasisCode;

                    // $ticket['Ticket']['wsTicketRequest']["FareRule"]["WSFareRule"]["FareRuleDetail"] =$data['details']->booking_details->rest->FareRule->WSFareRule->FareRuleDetail;
            ...
            if ($details[0]->num_of_adults >= 1) {
                ...
                if (is_array($data->rest->FareBreakdown->WSPTCFare)) {
                    $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["Fare"]["BaseFare"] = $data->rest->FareBreakdown->WSPTCFare[0]->BaseFare / $details[0]->num_of_adults;
                    $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["Fare"]["Tax"] = $data->rest->FareBreakdown->WSPTCFare[0]->Tax / $details[0]->num_of_adults;
                    ...
                    $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["Fare"]["BaseFare"] = $data->rest->FareBreakdown->WSPTCFare->BaseFare / $details[0]->num_of_adults;
                    $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["Fare"]["Tax"] = $data->rest->FareBreakdown->WSPTCFare->Tax / $details[0]->num_of_adults;
                    $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["Fare"]["ServiceTax"] = $data->rest->Fare->ServiceTax / $details[0]->num_of_adults;
            ...
            while ($i < $details[0]->num_of_children) {
                ...
                $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["Type"] = "Adult";
                $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["DateOfBirth"] = date('c',strtotime($data['dob_a'][$j]));
                if (count($data->rest->FareBreakdown->WSPTCFare) == 1 && $data->rest->FareBreakdown->WSPTCFare[1]->PassengerType == "Child") {
                    $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["Fare"]["BaseFare"] = $data->rest->FareBreakdown->WSPTCFare[1]->BaseFare / $details[0]->num_of_children;
                    $ticket["Ticket"]["wsTicketRequest"]["Passenger"]["WSPassenger"][$i]["Fare"]["Tax"] = $data->rest->FareBreakdown->WSPTCFare[1]->Tax / $details[0]->num_of_children;
