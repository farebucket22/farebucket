...
class Cabs extends CI_Controller
{
   ...
   {    
        ...
        else
        {
            ...
            $search_result = $this->search_cabs($source_result,$destination_result,$data,2);
        }    
    } 
	...
	function destination_cities($source_id = null, $data = null, $check = null)
	{
        ...
        //$this->search_cabs($source_id,$res_destid,$data,$check);
   }
	...
	function search_cabs($src_id = null, $dest_id = null, $data = null, $check = null) 
	{
        $_SESSION['currentUrl'] = current_full_url();
   ...
   /******common function for booking*****/
    ...
    public function general_booking()
    {
        ...
        if(strlen($returnId) === 0){
            ...
            $splitReturnID = explode("FBCA",$returnId);
        ...
        if( $this->input->get(null, true) ){
            ...
            $guest = array(
                ...
                'CabRequiredOn' => $data['CabRequiredOn'],
            ...
            $compacts = array_sum($pax_info->compact);
            $sedans = array_sum($pax_info->sedan);
            $suvs = array_sum($pax_info->suv);
            // print_r($compacts);print_r($sedans);print_r($suvs);die;
            ...
            if( $compacts ){
                ...
                $model = "compact";
                $id = $this->cab_model->insert_details($compacts,$guest,$ret[$i],$model,$sum,$_SESSION['cab_total_fare'],$_SESSION['cabMinSlab']);
                $id_insert = $this->cab_model->insert_booking_details($compacts,$guest,$ret[$i],$model,$sum,$_SESSION['cab_total_fare'],$_SESSION['cabMinSlab']);
	...
	function confirm_booking($ref_id,$id)
	{
    	...
    	//  $this->load->view("common/header.php");
        //  $this->load->view('cabs/multiway_tickets',array('data' => $header['ConfirmBooking_ByReferenceIDResult']));
        //  $this->load->view("common/footer.php");
        //  $this->search_book($header['ConfirmBooking_ByReferenceIDResult']);/*WI2014923-546*/
	}
    ...
    function cancel_booking()
    {
        ...
        if($header['CancelBooking_ByBookingNoResult'] == "Successfully Cancelled.")/*check $res variable*/
        {
            ...
            if( $this->input->post('confirm_ref_id') ){
                ...
                $mail_chk = $this->send_cancellation_email( $email_id, $booking_ref_id, $_SESSION['user_details'][0]->user_first_name);
        ...
        else
        {
            ...
            if( $this->input->post('booking_ref_id') ){
                ...
                echo "<pre>";
                print_r($header['CancelBooking_ByBookingNoResult']);die;
                redirect('admin/cabs');
            }
        }
    ...
    public function send_cancellation_email($email_id, $booking_ref_id, $name){
            $to = $email_id;
            //define the subject of the email
            $subject = 'Cancellation email'; 
            //define the message to be sent. Each line should be separated with \n

            $message = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head> <meta name="viewport" content="width=device-width"/> <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/> <title>Farebucket</title> <link rel="stylesheet" href="http://www.farebucket.com/css/email.css"></head><body bgcolor="#FFFFFF"> <table class="head-wrap"> <tr> <td></td><td class="header container"> <div class="content"> <table> <tr> <td><img src="http://www.farebucket.com/img/logo.png"/> </td><td align="right"> <h6 class="collapse"></h6> </td></tr></table> </div></td><td></td></tr></table> <hr/> <table class="body-wrap"> <tr> <td></td><td class="container" bgcolor="#FFFFFF"> <div class="content"> <table> <tr> <td> <h3>Hi, '.$name.'</h3> <p class="lead">Your ticket with booking id '.$booking_ref_id.', Has now been cancelled. The refund amount, if any, will be credited back to you in 7-8 working days. Please contact customer support at Phone num: +91-1234567890, Email ID: admin@farebucket.com for any further assistance.</p> <hr/> <table class="social" width="100%"> <tr> <td> <table align="left" class="column"> <tr> <td> <h5 class="">Contact Info:</h5> <p>Phone: <strong>1234567890</strong> <br/>Email: <strong><a href="emailto:hseldon@trantor.com">admin@farebucket.com</a></strong> </p></td></tr></table><span class="clear"></span> </td></tr></table> <hr/> </td></tr></table> </div></td><td></td></tr></table> <table class="footer-wrap"> <tr> <td></td><td class="container"> <div class="content"> <table> <tr> <td>© 2013–2015 Reddytrip LLP.</td></tr></table> </div></td><td></td></tr></table></body></html>';
            //define the headers we want passed. Note that they are separated with \r\n
            $headers = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
            $headers .= "From: Admin <info@farebucket.com>" . "\r\n";
