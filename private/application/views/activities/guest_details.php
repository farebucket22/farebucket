...
<?php
    ...
    $total_activity_fare = floor($cc + $booking_amount);
?>
<style>
    .reg_link{
    text-align: right;
...
<div class="wrap">
    ...
    <div class="container-fluid main clear-top">
        <div class="row guestDetailsCheckoutArea">
            <form class="col-xs-24 col-sm-12 col-sm-offset-1 col-md-12 col-md-offset-1 guestDetailsContainer" id="guestDetailsForm" method="POST" action="<?php echo site_url('activity/create_activity_booking');?>">
                ...
                ?>
                    ...
                    <input type="hidden" name="bookingAmount" value="<?php echo $total_activity_fare; ?>" />
        ...
        <div class="col-xs-24 col-sm-6 col-md-6 bookingDetailsContainer">
            ...
            <!-- <div class="col-xs-24 col-sm-10 col-md-10 bookingDetailsContainer">
                <div class="row">
                    <div class="col-xs-24 col-sm-14 bookingActivityDetails">
                        <div class="row bookingActivityName"><?php echo $activity_name; ?></div>
                        <div class="row bookingActivityLocationName"><?php echo $activity_location_name; ?></div>
                    ...
                    <div class="col-xs-24 col-sm-9 bookingSelectionDetails">
                        <div class="row bookingDateDetails bookingSelectionDetail">Booking Date: <b><?php echo date("d-m-Y", strtotime( $booking_date)); ?></b></div>
                        <div class="row bookingAdultDetails bookingSelectionDetail">No. of Adults: <b><?php echo $adult_count; ?></b></div>
                        <div class="row bookingKidDetails bookingSelectionDetail">No. of Children: <b><?php echo $child_count; ?></b></div>
                        <div class="row"><div class="form-group"><label for="tnc" class="control-label bookingSelectionDetail termncond"><input type="checkbox" name="tnc"/> I accept the <a href='#'>Terms and Conditions.</a></label></div></div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-xs-24 discountCodeError">Discount Code does not exist.</div>
                    <ul class="col-xs-4 pull-right bookingDiscountContainer">                        
                        <li>&#x20B9;</li>
                        <li class="bookingSelectionAmount"><?php echo number_format($booking_amount, 2); ?></li>                        
                    </ul>
                </div>
                <div class="row">
                    <div class="col-xs-12">
                        <div class="row">
                            <input type="text" class="col-xs-16 discountCodeField" placeholder="Discount Code" />
                            <button type="button" class="col-xs-16 bookingBtn applyDiscountCodeBtn bookingFormField"><span class="glyphicon glyphicon-circle-arrow-down"></span>&nbsp;APPLY</button>
                        </div>
                    </div>
                    <div class="col-xs-12">
                        <div class="row">
                            <ul class="col-xs-24 pull-right bookingSelectionAmountContainer">                        
                                <li>&#x20B9;</li>
                                <li class="bookingSelectionAmount"><?php echo number_format($booking_amount, 2); ?></li>                        
                            </ul>
                            <button type="button" class="col-xs-16 bookingBtn activitiesCheckoutBtn bookingFormField pull-right"><span class="glyphicon glyphicon-circle-arrow-right"></span>&nbsp;CHECKOUT</button>
                        </div>
                    </div>    
                </div>
            </div> -->
...
// this part checks the placeholder of a field and add various validation options eg: is e-mail validation for e-mail etc
    ...
    // $(".applyDiscountCodeBtn").click(function(){
    //     if(discountBtnFlag === 0){
    //         var discountCode = $(".discountCodeField").val();
    ...
    //         $.ajax({
    //             type: "POST",
    //             url: "<?php echo site_url('activity/apply_discount_code'); ?>",
    //             data: { discountCode: discountCode, discountModule: discountModule }
    //         })
    //         .done(function(data){
    //             data = $.parseJSON(data);
    //             if(data !== "Failure"){
    //                 var discountCodeValue = data.discount_code_value;
    //                 var discountCodeType = data.discount_code_type;
    //                 var activityAmount = <?php echo intval($booking_amount); ?>;
    //                 $.ajax({
    //                     type: "POST",
    //                     url: "<?php echo site_url('activity/calculate_discount_amount') ?>",
    //                     data: { discountCodeValue: discountCodeValue, discountCodeType: discountCodeType, activityAmount: activityAmount }
    //                 })
    //                 .done(function(data){
    //                     var tempArr = data.split("-");
    //                     var discountAmount = tempArr[0];
    //                     var finalAmount = tempArr[1];
    //                     var oldAmount = $(".bookingSelectionAmountContainer li:nth-child(2)").val();

    //                     $(".bookingDiscountContainer").show(function(){
    //                         $("this li:nth-child(2)").html(oldAmount);
    //                     });
    //                     $(".bookingSelectionAmountContainer li:nth-child(2)").html(finalAmount);
    //                     $(".applyDiscountCodeBtn").html('<span class="glyphicon glyphicon-remove-circle"></span>&nbsp;CANCEL');
    //                     $(".discountCodeError").hide();
    //                     discountBtnFlag = 1;
    //                 });
    //             } else{
    //                 alert('The discount code you have entered is Invalid.');
    //                 return false;
    //             }
    //         });
    //     } else{
    //         var oldAmount = $(".bookingDiscountContainer li:nth-child(2)").html();
    //         $(".bookingDiscountContainer").hide();
    //         $(".discountCodeField").val("");
    //         $(".bookingSelectionAmountContainer li:nth-child(2)").html(oldAmount);
    //         $(".applyDiscountCodeBtn").html('<span class="glyphicon glyphicon-circle-arrow-down"></span>&nbsp;APPLY');
    //         discountBtnFlag = 0;
    //     }
...
//discount section
    ...
    var discountClick = 0;
    var total_fare = parseInt("<?php echo $booking_amount;?>");
