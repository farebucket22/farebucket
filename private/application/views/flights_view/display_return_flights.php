...
// remove last sorting icon end
    ...
    $('#search tbody').on('click', '.book_btn', function(){
        ...
        if( click_count == 1 ){
            ...
            $('.fl_btn2').show();
            $('.fl_septr2').show();
            // $('.airline-name1').html($('#row-'+index+' td').eq(0).html());
            // $('.duration1').html($('#row-'+index+' td').eq(3).html())
        } else {
            ...
            // $('.airline-name').html($('#row-'+index+' td').eq(0).html());
            // $('.duration').html($('#row-'+index+' td').eq(3).html())
        }
    ...
    var cab_search_parameters = 
    {
        ...
        adult_count: "<?php echo $_GET['adult_count'];?>", 
        youth_count: "<?php echo $_GET['youth_count'];?>",
        journey_date: "<?php echo $_GET['to_date'];?>",
    }
    $('#cab_spinner').show();
    $.ajax({
    ...
    .done(function (cabData){
        ...
        if(result !== false)
        {
            var search_base = "<?php echo site_url('cab_api/cabs/flights_to_cabs?source='.$_GET['from'].'&destination='.$_GET['to'].'&adult_count='.$_GET['adult_count'].'&youth_count='.$_GET['youth_count'].'&journey_date='.$_GET['to_date'].'&oneway_change_cab=true&total_count='+'total_count');?>";
    ...
    /*========Alternate buttons function end==========*/
    ...
    $.ajax({
        ...
        //url: "<?php echo site_url('flights_api/api/return_search');?>",
        type: "POST",
