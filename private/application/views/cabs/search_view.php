...
// validation part
    ...
    .done(function(retDat){
        ...
        $.each(retDat.statename, function(i, val){
            if( get_source === val ){
                sel_opt.append("<option id='"+retDat.stateid[i]+"' value='"+retDat.stateid[i]+"-"+retDat.statename[i]+"' selected='selected'>"+val+"</option>");
            }else{
...
<?php if( isset($_GET['flight_type']) && $_GET['flight_type'] == 'outstation' ):?>
    ...
    .done(function(retDat){
        ...
        $.each(retDat.statename, function(i, val){
            console.log( 'ajax:'+val+' get_dest:'+get_destination );
