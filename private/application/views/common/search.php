    ...
    <form class="row wrapper activitySearchForm" action="<?php echo site_url('activity/get_search_results'); ?>" method="GET" >
        ...
        <div class="col-xs-20 col-sm-9">
            <select class="citySelect extra-margin" name="citySelect">
                ...
                <?php
                foreach($city_data as $cd1) {
                    if(isset($_SESSION['city_id'])){
                        if($cd1->activity_city_id==$_SESSION['city_id']){
                            echo "<option id='".$cd1->activity_city_id."' value='".$cd1->activity_city_id."-".$cd1->activity_city_name."' selected='selected'>".$cd1->activity_city_name."</option>";
                        } else{
                            echo "<option id='".$cd1->activity_city_id."' value='".$cd1->activity_city_id."-".$cd1->activity_city_name."'>".$cd1->activity_city_name."</option>";
                        }
                    } else{
            ...
            url: '<?php echo site_url(); ?>/activity/get_cities',
                ...
                success: function(data) {
                    ...
                    $.each(citiesData,function(){
                        ...
                        $("select.citySelect").append($('<option id="'+this.activity_city_id+'" value="'+this.activity_city_id+'-'+this.activity_city_name+'">', { value : this.activity_city_id })
                                        .text(this.activity_city_name));
